...
| DATA-01 | Migration-forward and preserved-row proof | Data owner/operator | Execute additive migrations 001–003 on a disposable Neon branch, compare Auth/legacy/V2 counts and schema, record rollback/disposal result | `manual` | Migration 003 is drafted and statically reviewed but not applied; never run against production/default branch without explicit approval |
...
| DATA-03 | Append-only wallet ledger | Server/data owner | Database policy/trigger or restricted writer boundary, with update/delete denial tests | `partial` | Migration 003 drafts the append-only trigger; disposable-branch application and denial proof remain open |
| TX-01 | Atomic QR replay transition | Server/data owner | Conditional update/row-lock transaction, concurrent first-pass/second-pass test and audit append proof | `partial` | Pure QR policy and migration-003 conditional function exist; disposable-branch execution and concurrency proof remain open |
| TX-02 | Transaction state transition authority | Server/data owner | Positive/negative state-transition matrix across intent, QR, payment, fulfilment, received, rating and close | `todo` | Do not build transaction UI against unproven transitions |
...
Local proof currently reports 10 Vitest files and 37 passing tests, a successful production build, 3 bundled Vercel functions, a clean client boundary, map/route policy proof, QR and transaction state-policy proof, Auth fail-closed malformed-token proof, public stock-field absence, additive migration static review and read-only Neon/API smoke evidence. These are supporting foundations, not closure of the open rows above.
