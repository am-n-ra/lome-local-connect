...
| DATA-01 | Migration-forward and preserved-row proof | Data owner/operator | Execute additive migrations 001–003 on a disposable Neon branch, compare Auth/legacy/V2 counts and schema, record rollback/disposal result | `partial` | Count-level comparison and aggregate Auth ID/schema checksums are complete; row-level/business-record checksum, branch disposal and production/default-branch decision remain manual |
...
| TX-01 | Atomic QR replay transition | Server/data owner | Conditional update/row-lock transaction, concurrent first-pass/second-pass test and audit append proof | `partial` | Disposable branch proves first-pass/second-pass behavior and QR primitive presence; concurrent execution and audit append proof remain open |
