...
| DATA-01 | Migration-forward and preserved-row proof | Data owner/operator | Execute additive migrations 001–003 on a disposable Neon branch, compare Auth/legacy/V2 counts and schema, record rollback/disposal result | `partial` | Count-level comparison is complete: 35 Auth users, 26 V2 tables and 0 selected legacy tables are unchanged; row-level/checksum comparison, branch disposal and production/default-branch decision remain manual |
