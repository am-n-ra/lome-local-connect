...
| DATA-01 | Migration-forward and preserved-row proof | Data owner/operator | Execute additive migration on a disposable Neon branch, compare Auth/legacy/V2 counts and schema, record rollback/disposal result | `manual` | Never run against production/default branch without explicit approval |
...
| DATA-03 | Append-only wallet ledger | Server/data owner | Database policy/trigger or restricted writer boundary, with update/delete denial tests | `todo` | No wallet mutation or migration until the boundary is reviewed and approved |
| TX-01 | Atomic QR replay transition | Server/data owner | Conditional update/row-lock transaction, concurrent first-pass/second-pass test and audit append proof | `partial` | Pure QR policy exists; no token mutation or concurrency proof exists |
...
Local proof currently reports 9 Vitest files and 33 passing tests, a successful production build, 3 bundled Vercel functions, a clean client boundary, map/route policy proof, QR policy proof, Auth fail-closed malformed-token proof, public stock-field absence and read-only Neon/API smoke evidence. These are supporting foundations, not closure of the open rows above.
