...
## 12. 2026-08-24 — GitHub-linked storage verification superseding the pre-provisioning note

The earlier statement that the Vercel Blob store was not provisioned is superseded by the owner’s manual provisioning action and the following bounded runtime proof. The UUID-shaped persisted public fixture was accepted by the corrected claim route after commit `5aec139` was pushed to `am-n-ra/lome-local-connect` on `omni-v2-rebuild`, the Vercel-linked branch. The canonical alias served the updated route through Vercel; direct connector deployment metadata was unavailable during this window because the Vercel service was under maintenance, so no exact GitHub-triggered deployment ID is recorded.

An owner-authorized empty draft was opened for the public unclaimed facility `Marche de Hanoukope`. The Claim Heartwood sheet displayed six typed private-evidence categories and `Preuves privées, statut séparé`, demonstrating that the production runtime recognizes private storage as available without exposing a token. No file was selected, no upload or submit occurred, and the draft was cancelled immediately. The facility returned to `unclaimed` and the public sheet again offered only a new claim start. This verifies the provider-ready boundary and reversible draft operation; it does not verify file upload, private object reads, metadata reconciliation, active operator/reviewer authorization, claim submission, review or notification delivery.

**Updated Ring decision:** `partial / provider-ready, role and evidence path still open`. The next gate remains explicit owner selection and authorization of the first trusted Omni `operator` and/or `reviewer` account(s), followed by a separately authorized bounded OSM source batch. Neon Auth `admin` remains distinct from the Omni business roles; no role row was changed in this checkpoint. Global Root remains `review`; no production-readiness claim is made.

## 13. 2026-08-24 — Bounded private object and submitted-claim proof

The first live upload attempt exposed a Vercel Blob protocol mismatch: the client token was nested in Omni’s generic response envelope. Commit `cfbd3da` returned the provider callback response at the top level and was deployed from GitHub as READY deployment `dpl_HCr8LaWg3n1LJFycmTYE7qRdSGUP` on `omni-v2-rebuild`. A subsequent resume-state regression was fixed in commit `973b6bc`, deployed as READY deployment `dpl_82TxYc43dCHQAEV29CKwbHXDLKVr`; its public serializer maps internal `verification_draft` to public `unclaimed` semantics and its regression test passed.

On the refreshed canonical deployment, the existing draft for `Marche de Hanoukope` was reopened without another facility creation. The single attached manga screenshot was used as a non-sensitive storage test artifact only and uploaded under the typed `identity` category. The UI reported one private file and a private reference verifiable by Omni. After explicit point-of-action confirmation, submission succeeded. Aggregate verification on persistent V2 branch `br-dawn-hill-am5amy22` recorded one submitted request, one private evidence object and facility `verification_submitted`. No reviewer role was present, so reviewer event and in-app delivery counts were zero; no review, role grant or OSM import was attempted.

This closes the bounded claimant upload → provider verification → submitted-claim vertical proof, but not reviewer private GET/read, reviewer queue authorization, reviewer decision, notification delivery, retention/reconciliation, role bootstrap or operational OSM import. The test claim is retained as auditable historical data and is not certification, ownership or marketplace evidence. Global Root remains `review`; production readiness is not claimed.
