...
The focused Root tests cover authorized seller and buyer transitions, invalid state jumps, wrong-role membership, missing membership and system-only closure. The full repository pass reports 12 Vitest files and 75 passing tests, a successful TypeScript/Vite build, 10 bundled Vercel functions and `Client boundary: clean`. Wallet balance and bonus-grant evidence is maintained separately in [`v2-root-wallet-evidence.md`](./v2-root-wallet-evidence.md).
...
Focused repository tests cover an authorized seller transition, stale or unauthorized rejection, canonical retry response and the transaction/audit conflict boundaries. This remains a local repository seam; the separate persistent-V2 demo fixture records bounded transaction events and audit facts, but no live bearer-backed transition request was executed and the fixture is not a real sale.
...
This artifact proves the pure state-policy boundary and the local repository seam only. It does not prove that a live bearer-backed Neon mutation reads the current persisted state, locks or conditionally updates the transaction row, appends the canonical transaction event exactly once, or returns the original authoritative result on duplicate/retry. The bounded demo fixture is recorded separately and does not replace live API, concurrency or recovery proof.
