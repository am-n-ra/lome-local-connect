...
After commit `b83cc20`, the canonical deployment was smoke-tested again: `POST /api/v2/availability-responses` with an empty body returned `401 AUTH_REQUIRED`, and `POST /api/v2/qr-issuances` with an empty body returned `401 AUTH_REQUIRED`. The prior `404` responses were therefore identified as missing Vercel entrypoints and corrected. These probes made no mutation and prove route presence plus fail-closed outer authorization only.
