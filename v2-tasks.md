...
| Task ID | Task | Dependencies | Status |
|---|---|---|---|
| CANOPY-001 | Reconcile all surfaces against Species blueprint | Required branches | `partial` | Buyer map-first structure now inherits the V4.1 white/black/gray map direction, neutral source-backed density marks, native free camera ownership with retained idle resume, bounded search reveal, explicit result toolbar, intentional desktop bottom-sheet composition and the documented permission-aware arrival contract; Seller/Reviewer/Inbox mini-rings are audited read-only, while the full authenticated compact matrix remains open |
...
| CANOPY-004 | Map, API, database, cache and bundle performance audit | Required branches | `partial` | Prior source validation passed 119 tests, build bundling exactly 12 Vercel functions and clean client boundary; V4.1 source changes add a pure globe-axis helper/test and still require a fresh baseline, canonical monochrome capture, remote-tile reliability and deeper performance/recovery review |
