...
| ROOT-008 | Define private media/evidence storage boundary | ROOT-003 | `partial / blocked` | Shared evidence contract and Vercel Blob private adapter now enforce claimant token authorization, path binding, MIME/10 MB limits, provider metadata verification and authenticated no-store reads; private store provisioning, access-control and upload/download proof remain blocked |
...
| TRUST-004 | Capture identity, company/facility, product and location evidence | TRUST-002/003 | `partial / blocked` | J5 sheet has six typed upload categories, progress/error/removal states and private-only copy; actual provider-backed capture awaits Blob store provisioning and active claimant role |
| TRUST-005 | Persist/edit/cancel/resume evidence and submit idempotently | TRUST-004 | `partial / blocked` | Versioned draft, claimant-owned pre-review cancel, direct private upload, provider object verification and multiplexed submit route exist; real upload, resume/idempotency and live submit proof await provider provisioning and a trusted claimant session |
