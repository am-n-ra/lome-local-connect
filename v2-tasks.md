...
| ROOT-005 | Add wallet, entitlement, transaction, QR and fulfilment contracts | ROOT-003 | `review` | Root/Flow contracts plus QR and transaction state-policy tests cover actor ownership, approved transitions, match, expiry and replay denials; ledger, state persistence and atomic QR mutation tests remain required |
| ROOT-006 | Add audit events, correlation IDs and mutation idempotency | ROOT-001 | `review` | 9 test files/34 tests pass, including duplicate, audit, public-boundary, QR policy, Auth-boundary and state-policy helpers; database concurrency proof remains open |
