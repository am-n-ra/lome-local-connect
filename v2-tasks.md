...
| AUTH-03 | Recreate only authorized demo Auth identities for live bearer proof | ROOT-001, ROOT-002 | `blocked` | [`v2-root-demo-recreation-decision.md`](./v2-root-demo-recreation-decision.md) records the confirmed scope and observed post-deletion state: Auth users decreased from 35 to 30, the buyer-ready fixture remains linked, and the seller-ready V2 account has a stale/unmatched Auth binding while its facility/product rows remain. The next valid path is user-controlled sign-up/sign-in, encrypted CI/Preview secrets or a configured Auth admin/test facility; direct SQL and agent password handling remain prohibited |
