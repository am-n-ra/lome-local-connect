...
| ROOT-002 | Link Neon Auth identity to idempotent Omni account provisioning | ROOT-001 | `partial` | 35 Auth users were preserved; persistent V2 now has one buyer-ready account from the confirmed KH availability proof and one seller-ready account bound to an existing demo-like Auth identity under the explicitly authorized fixture scope. The availability writer provisions buyer account/wallet idempotently; seller binding is bounded fixture evidence, not production onboarding or adoption |
...
| ROOT-004 | Add facility, trust, catalogue and availability contracts | ROOT-003 | `partial` | 75-test suite plus canonical facility detail HTTP 200; branch serializer/SQL scan proves no public stock field; actual availability writer validates product/facility publication and trust, returns canonical replay and rejects shape mismatch; authenticated browser proof and aggregate persistence verification passed on persistent V2 with one request after two submissions; the seller response validates owned facility, published product, request scope and allocated quantity and is proven with one bounded response; recovery and deployed seller route proof remain open |
...
| ROOT-013 | Complete seller availability response and server-issued QR seams | ROOT-004, ROOT-005, ROOT-006 | `partial` | [`v2-root-seller-response.md`](./v2-root-seller-response.md) defines server seller ownership, scope, publication, allocation and response idempotency; source tests pass and persistent bounded evidence proves response → snapshot → QR first pass/replay rejection → declaration → seller acknowledgement; deployed seller bearer proof, concurrency and camera recovery remain open |
| SEC-001 | Rotate persistent-V2 credential, update Vercel binding and preserve secret hygiene | ROOT-001, ROOT-003 | `partial` | [`v2-root-credential-rotation-evidence.md`](./v2-root-credential-rotation-evidence.md) records successful Neon reset, Vercel Production+Preview update, READY canonical redeploy and aggregate/public/protected smoke checks; follow-up secret-safe rotation remains required because the replacement was briefly rendered in browser extraction output |
