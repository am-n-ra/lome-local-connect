...
| Task ID | Task | Dependencies | Status |
|---|---|---|---|
| CANOPY-001 | Reconcile all surfaces against Species blueprint | Required branches | `partial` | Buyer map-first structure, contextual nearby surface, contained idle globe, interruptible hover pause/resume, bounded search reveal, visible single pin/cluster renderer, authenticated Buyer result and Seller/Reviewer/Inbox mini-rings audited; authenticated compact surface matrix remains open |
| CANOPY-002 | Four-width responsive and safe-area certification | Required branches | `partial` | Public Playwright passes `390×844` normal/reduced-motion canvas, dock containment, controls, zoom and no horizontal overflow; authenticated Buyer/Seller/Reviewer/Inbox surfaces remain proven at the available wide Sandbox viewport, while authenticated compact result-sheet and full Seller/Reviewer matrix remain open |
| CANOPY-003 | Accessibility, keyboard, focus and reduced-motion certification | Required branches | `partial` | Native accessible pin, named zoom/location controls, one visible focus-name proof and public reduced-motion no-rotation proof pass; full Tab/Shift+Tab, focus-trap, authenticated compact traversal and context-motion capture remain |
| CANOPY-004 | Map, API, database, cache and bundle performance audit | Required branches | `partial` | 12-function build, client boundary, canonical map/fallback/pin checks, query-only request and no continuous reveal bounds cadence pass; bundle warning, tile reliability and deeper performance/recovery checks remain |
