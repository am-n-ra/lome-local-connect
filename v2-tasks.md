...
| ROOT-002 | Link Neon Auth identity to idempotent Omni account provisioning | ROOT-001 | `partial` | 35 Auth users and 0 persistent-V2 accounts observed; canonical trusted origin and branch JWKS configured/reachable; malformed credentials fail closed; the actual availability writer provisions account and wallet idempotently in one guarded statement; the confirmed live browser request/replay produced one account, one wallet and one request on production/default, while persistent V2 remained at zero; repeat against the intended V2 data boundary remains required |
...
The first execution batch is `SEED-001` through `ROOT-011`, followed by the buyer Trunk. Root evidence currently includes 11 local test files/68 tests, the recorded disposable-branch database behavior checks and one bounded live Auth/availability idempotency proof on production/default; no new seller, wallet, transaction or QR UI implementation should begin while the Root gate remains in review.
