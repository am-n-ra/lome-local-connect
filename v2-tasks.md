...
| ROOT-006 | Add audit events, correlation IDs and mutation idempotency | ROOT-001 | `partial` | 11 test files/60 tests pass, including redacted internal errors, typed malformed-input handling, availability replay-shape checks, Auth-linked conditional QR persistence, guarded purchase-intent replay checks, wallet spend/bonus replay checks and transaction event replay checks; the purchase-intent HTTP wrapper is bundled; disposable-branch guard behavior and QR first-pass/second-pass results remain; database concurrency, audit append and live idempotency proof remain open |
