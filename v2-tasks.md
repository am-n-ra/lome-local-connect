...
| Task ID | Task | Dependencies | Status | Gate evidence |
|---|---|---|---|---|
| TRUNK-MAP | Build persistent MapLibre globe/map arrival and camera ownership | ROOTS | `verified` | Canonical public proof plus authenticated 891×765 proof: persistent MapLibre canvas/globe, interruptible idle rotation, enabled zoom, public pin continuity and no continuous rotation-fetch loop; location success, full camera-priority interruption, remote raster reliability and recovery remain |
| TRUNK-SEARCH | Build one search pill/dock and one Options disclosure | TRUNK-MAP | `verified` | Authenticated canonical proof now shows query-aware loading → `nearby-state-ready` for `Marche de Hanoukope` through a query-only global request; anonymous Auth boundary and separated dock/sheet are also proven. Empty/error/recovery, keyboard and full context-motion proof remain |
...
| Task ID | Task | Dependencies | Status |
|---|---|---|---|
| CANOPY-001 | Reconcile all surfaces against Species blueprint | Required branches | `partial` | Buyer map-first structure, contextual nearby surface, contained idle globe, visible pins/clusters, authenticated Buyer result and Seller/Reviewer/Inbox mini-rings audited; full surface matrix remains open |
| CANOPY-002 | Four-width responsive and safe-area certification | Required branches | `partial` | Buyer six viewport audit passes idle geometry, no overflow and zoom; authenticated Buyer/Seller/Reviewer/Inbox surfaces are proven at the available 891×765 Sandbox viewport, while compact-width authenticated and full Seller matrix remain open |
| CANOPY-003 | Accessibility, keyboard, focus and reduced-motion certification | Required branches | `partial` | Native accessible pins, live named controls and reduced-motion state are proven; full Tab/Shift+Tab, focus-trap and authenticated context-motion capture remain |
...
| CANOPY-005 | Dead-action, false-state, privacy and client-boundary audit | Required branches | `partial` | No stock/trust/permission inference, private handoff stays locked, client boundary passes and no protected write occurred in this audit; broader negative-state and facility-focus sweep remains |
