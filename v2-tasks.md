...
| Task ID | Task | Dependencies | Status |
|---|---|---|---|
| CANOPY-001 | Reconcile all surfaces against Species blueprint | Required branches | `partial` | Buyer map-first structure now includes the darker clean-edge `deep-neutral` treatment, concentric density rings, native free camera ownership with retained idle resume, bounded search reveal, explicit result toolbar, intentional desktop bottom-sheet composition, and the documented permission-aware arrival contract; Seller/Reviewer/Inbox mini-rings are audited read-only, while the full authenticated compact matrix remains open |
...
| CANOPY-003 | Accessibility, keyboard, focus and reduced-motion certification | Required branches | `partial` | Native accessible pin/cluster ring, named zoom/location controls, production synthetic granted marker, honest denied/timeout states, robust normal rotation proof and public reduced-motion no-rotation proof pass; full Tab/Shift+Tab, focus-trap, authenticated compact traversal and screen-reader review remain |
| CANOPY-004 | Map, API, database, cache and bundle performance audit | Required branches | `partial` | Final source validation passes 116 tests, build bundling exactly 12 Vercel functions and clean client boundary; production camera proof confirms globe→mercator at local zoom and search proof confirms one query-only request path; Vite chunk warning, remote tile reliability and deeper performance/recovery remain |
| CANOPY-005 | Dead-action, false-state, privacy and client-boundary audit | Required branches | `partial` | Rings remain density-only, pins remain public presence only, denied/timeout location stays honest, synthetic granted location is explicitly bounded, same-query re-entry now resolves, result refine/exit actions are real, multi-product remains a planning note only, and no protected write occurred; full negative-state/facility-focus sweep remains |
