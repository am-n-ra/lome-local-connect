...
| ROOT-003 | Define domain ownership and persistence model | ROOT-001 | `review` | Static review plus isolated-branch catalog of 26 V2 tables/125 constraints; disposable branch denies mismatched facility/company ownership and availability scope; preserved-row and live transaction enforcement remain open |
...
| ROOT-006 | Add audit events, correlation IDs and mutation idempotency | ROOT-001 | `review` | 10 test files/37 tests pass, plus disposable-branch guard behavior and QR first-pass/second-pass results; database concurrency, audit append and live idempotency proof remain open |
...
| ROOT-009 | Preserve Auth identities and legacy records during additive migration | ROOT-003 | `partial` | Migration 003 executed on expiring disposable branch with 2 constraints, 7 triggers and QR function; read-only coexistence recorded; preserved-row comparison and production/default decision remain required |
