...
| ROOT-002 | Link Neon Auth identity to idempotent Omni account provisioning | ROOT-001 | `partial` | 35 Auth users and 0 persistent-V2 accounts were observed before the proof; canonical trusted origin and branch JWKS are configured/reachable; malformed credentials fail closed; the availability writer provisions account and wallet idempotently in one guarded statement; after adding Vercel `V2_DATABASE_URL`, the confirmed live browser request/replay produced one account, one wallet and one request on persistent V2 |
...
| ROOT-004 | Add facility, trust, catalogue and availability contracts | ROOT-003 | `partial` | 68-test suite plus canonical facility detail HTTP 200; branch serializer/SQL scan proves no public stock field; actual availability writer validates product/facility publication and trust, returns canonical replay and rejects shape mismatch; after the Vercel binding correction, authenticated browser proof and aggregate persistence verification passed on persistent V2 with one request after two submissions; recovery remains open |
| ROOT-005 | Add wallet, entitlement, transaction, QR and fulfilment contracts | ROOT-003 | `partial` | Root/Flow contracts plus QR and transaction state-policy tests cover actor ownership, approved transitions, match, expiry and replay denials; actual repository seams now cover conditional QR verification, guarded purchase-intent scaffolding, append-only wallet spending, one-time confirmed-trust/three-sale bonus granting, conditional buyer/seller transaction-state events, declaration-only cash/mobile-money/pay-on-delivery payment and seller confirmation with immutable snapshot, membership and initial event; authenticated `POST /api/v2/purchase-intents`, `POST /api/v2/transaction-transitions`, `POST /api/v2/external-payment-declarations` and `POST /api/v2/external-payment-confirmations` are included in the eight-function serverless bundle; [`v2-root-wallet-evidence.md`](./v2-root-wallet-evidence.md) and [`v2-root-payment-evidence.md`](./v2-root-payment-evidence.md) record wallet, bonus and external-payment boundaries; disposable branch denies ledger update/delete and proves QR first-pass/second-pass behavior; live state persistence and concurrency remain required |
| ROOT-006 | Add audit events, correlation IDs and mutation idempotency | ROOT-001 | `partial` | 11 test files/68 tests pass, including redacted internal errors, typed malformed-input handling, availability replay-shape checks, Auth-linked conditional QR persistence, guarded purchase-intent replay checks, wallet spend/bonus replay checks, transaction event/audit append conflict checks, declaration-only payment replay/authorization plus HTTP policy checks and seller confirmation gating/replay checks; the purchase-intent, transaction-transition, external-payment declaration, external-payment confirmation and QR verification wrappers are bundled in the eight-function build; disposable-branch guard behavior and QR first-pass/second-pass results remain; sequential live availability idempotency is now evidenced on persistent V2, while database concurrency, live audit append and broader live mutation replay remain open |
...
| ROOT-009 | Preserve Auth identities and legacy records during additive migration | ROOT-003 | `partial` | Migration 003 executed on expiring disposable branch with 2 constraints, 7 triggers and QR function; read-only Auth/schema/public-row coexistence and exact fixture inventory recorded; preserved-row rollback/recovery and production/default decision remain required |
...
The first execution batch is `SEED-001` through `ROOT-011`, followed by the buyer Trunk. Root evidence currently includes 11 local test files/68 tests, the recorded disposable-branch database behavior checks, one bounded live Auth/availability idempotency proof on production/default and one corrected proof on persistent V2; no new seller, wallet, transaction or QR UI implementation should begin while the Root gate remains in review.
