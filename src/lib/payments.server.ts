...
export async function startDeposit(input: {
  ...
  const deposit = await queryOne<{ id: string }>(
    `INSERT INTO public.wallet_deposits (facility_id, user_id, amount)
     VALUES ($1, $2, $3) RETURNING id`,
    [input.facilityId, input.userId, input.amount],
  );
  ...
  await queryOne(
    `UPDATE public.wallet_deposits
     SET provider_txn_id = $1, checkout_url = $2, updated_at = now()
...
export async function reconcileDeposit(
  ...
  const fresh = await queryOne<DepositRow>(
    "SELECT * FROM public.wallet_deposits WHERE id = $1",
    [depositId],
  );
