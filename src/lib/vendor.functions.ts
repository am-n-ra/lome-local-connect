...
/** Mode démo : rechargement du portefeuille (FedaPay branché ultérieurement). */
export const topUpWallet = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertOwner(context.userId, data.facilityId);
    const subscription = await ensureSubscription(data.facilityId);
    const qualifies = data.amount >= QUALIFYING_AMOUNT;
    const updated = await queryOne<VendorSubscription>(
      `UPDATE public.subscriptions
       SET wallet_balance = wallet_balance + $1,
           tier = CASE WHEN $2 THEN 'pro' ELSE tier END,
           pro_active_until = CASE WHEN $2 THEN $3::date ELSE pro_active_until END,
           last_qualifying_action_month = CASE WHEN $2 THEN $4 ELSE last_qualifying_action_month END
       WHERE facility_id = $5 RETURNING *`,
      [
        data.amount,
        qualifies,
        extendedProUntil(subscription.pro_active_until),
    ...
    await writeAudit(context.userId, "wallet.topup", "subscription", data.facilityId, {
      amount: data.amount,
      mode: "demo",
    });
    return updated!;
