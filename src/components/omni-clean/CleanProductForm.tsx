...
export function CleanProductForm({ atProductCap, onSubmit }: Props) {
  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="space-y-5 rounded-[1.4rem] border border-black/5 bg-white/65 p-4 sm:p-5">
      ...
      {advanced ? <div className="grid gap-3 rounded-2xl border border-black/5 bg-white/60 p-3 sm:grid-cols-2"><label className="space-y-1.5"><span className="text-xs font-bold">Stock visible sur Omni (%)</span><input className="omni-clean-field" inputMode="numeric" type="number" min="0" max="100" value={draft.allocation} onChange={(event) => patch({ allocation: event.target.value })} /></label><label className="space-y-1.5"><span className="text-xs font-bold">Publication</span><select className="omni-clean-field" value={draft.status} onChange={(event) => patch({ status: event.target.value as SellerProductDraft["status"] })}><option value="active">Actif</option><option value="draft">Brouillon</option><option value="paused">En pause</option><option value="sold_out">Épuisé</option></select></label></div> : null}
