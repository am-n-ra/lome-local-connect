  ...
  return (
    <form onSubmit={(event) => void submit(event)} className="space-y-5 rounded-[1.4rem] border border-black/5 bg-white/65 p-4 sm:p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="space-y-1.5 sm:col-span-2"><span className="text-xs font-extrabold">Nom du produit</span><input className="omni-clean-field text-base" value={draft.name} onChange={(event) => patch({ name: event.target.value })} placeholder="Ex. Ciment 50 kg" autoComplete="off" required /></label>
        <label className="space-y-1.5"><span className="text-xs font-extrabold">Prix public (FCFA)</span><input className="omni-clean-field text-base" inputMode="numeric" type="number" min="0" value={draft.price} onChange={(event) => patch({ price: event.target.value })} placeholder="12 000" required /></label>
        <label className="space-y-1.5"><span className="text-xs font-extrabold">Quantité disponible</span><input className="omni-clean-field text-base" inputMode="numeric" type="number" min="0" value={draft.quantity} onChange={(event) => patch({ quantity: event.target.value })} placeholder="1" required /></label>
        <label className="space-y-1.5 sm:col-span-2"><span className="flex items-center gap-2 text-xs font-extrabold"><Image className="h-3.5 w-3.5 text-[var(--omni-orange)]" />Photo ou média (facultatif)</span><input className="omni-clean-field text-base" value={draft.photoUrl} onChange={(event) => patch({ photoUrl: event.target.value })} placeholder="https://…" inputMode="url" aria-label="URL de la photo du produit" /></label>
      </div>
      ...
      <div className="rounded-2xl bg-[var(--omni-paper)] p-3"><div className="flex items-center gap-2"><Tag className="h-4 w-4 text-[var(--omni-orange)]" /><p className="text-xs font-extrabold">Offre et coupon Omni</p></div><p className="mt-1 text-xs leading-5 text-[var(--omni-ink-muted)]">La remise déclarée sera recalculée par le serveur avant l’intention d’achat. Le code public est facultatif ; sans code, l’offre reste honnêtement affichée comme une remise Omni sans coupon partageable.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="space-y-1.5"><span className="text-xs font-bold">Remise Omni (%)</span><input className="omni-clean-field text-base" inputMode="numeric" type="number" min="1" max="90" value={draft.couponPercent} onChange={(event) => patch({ couponPercent: event.target.value.replace(/\D/g, "") })} aria-label="Remise Omni en pourcentage" /></label><label className="space-y-1.5"><span className="text-xs font-bold">Code public (facultatif)</span><input className="omni-clean-field text-base uppercase" value={draft.couponCode} onChange={(event) => patch({ couponCode: event.target.value.toUpperCase() })} placeholder="Ex. BIENVENUE" maxLength={24} aria-label="Code coupon public" /></label><label className="space-y-1.5 sm:col-span-2"><span className="text-xs font-bold">Description du code (facultatif)</span><input className="omni-clean-field text-base" value={draft.couponDescription} onChange={(event) => patch({ couponDescription: event.target.value })} placeholder="Ex. Offre de bienvenue" maxLength={200} aria-label="Description du coupon public" /></label><div className="rounded-xl bg-white/70 p-3 text-xs leading-5 text-[var(--omni-ink-muted)] sm:col-span-2">{draft.couponCode.trim() ? `Aperçu buyer : ${draft.couponCode.trim().toUpperCase()} · −${draft.couponPercent || "0"}%` : "État affiché : offre Omni active, sans code public."}</div></div></div>
