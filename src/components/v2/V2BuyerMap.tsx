...
type Props = { facilities: Facility[]; selectedId: string | null; onSelect: (facility: Facility) => void; onBoundsChange?: (bounds: [number, number, number, number]) => void; onLocationState?: (state: "requesting" | "exact" | "approximate" | "denied" | "timeout" | "cancelled") => void };
...
export function V2BuyerMap({ facilities, selectedId, onSelect, onBoundsChange, onLocationState }: Props) {
  const container = useRef<HTMLDivElement>(null); const mapRef = useRef<Map | null>(null); const [locating, setLocating] = useState(false); const facilitiesRef = useRef(facilities); const rotating = useRef(true); const fallback = useRef(false); const boundsTimer = useRef<number | null>(null); const locationRequest = useRef<number | null>(null); facilitiesRef.current = facilities;
  const emitBounds = (map: Map) => { if (boundsTimer.current) window.clearTimeout(boundsTimer.current); boundsTimer.current = window.setTimeout(() => { const b = map.getBounds(); onBoundsChange?.([b.getWest(), b.getSouth(), b.getEast(), b.getNorth()]); }, 220); };
  useEffect(() => {
    if (!container.current || mapRef.current) return;
    const map = new Map({ container: container.current, style: REMOTE_STYLE, center: [1.22, 6.13], zoom: 1.35, attributionControl: { compact: true }, cooperativeGestures: true }); mapRef.current = map;
    const pause = () => { rotating.current = false; }; const resume = () => { if (map.getZoom() < 2.2) rotating.current = true; };
    map.on("mousedown", pause); map.on("touchstart", pause); map.on("wheel", pause); map.on("dragstart", pause); map.on("zoomstart", pause);
    map.on("move", () => emitBounds(map)); map.on("moveend", resume); map.on("error", () => { if (!fallback.current) { fallback.current = true; map.setStyle(FALLBACK_STYLE); } });
    map.on("load", () => { map.setProjection({ type: "globe" }); map.resize(); if (!map.getSource(SOURCE)) {
      ...
      map.addLayer({ id: "facility-selected-halo", type: "circle", source: SOURCE, filter: ["==", ["get", "id"], ""], paint: { "circle-color": "#e86f2b", "circle-radius": 18, "circle-opacity": 0.22, "circle-stroke-color": "#e86f2b", "circle-stroke-width": 1.5 } });
  ...
  const locate = () => { if (locationRequest.current !== null) return; setLocating(true); onLocationState?.("requesting"); if (!navigator.geolocation) { setLocating(false); return onLocationState?.("denied"); } locationRequest.current = window.setTimeout(() => { locationRequest.current = null; setLocating(false); }, 10000); navigator.geolocation.getCurrentPosition((p) => { if (locationRequest.current !== null) window.clearTimeout(locationRequest.current); locationRequest.current = null; setLocating(false); const approximate = p.coords.accuracy > 500; onLocationState?.(approximate ? "approximate" : "exact"); mapRef.current?.easeTo({ center: [p.coords.longitude, p.coords.latitude], zoom: approximate ? 5 : 7, duration: 900 }); }, (e) => { if (locationRequest.current !== null) window.clearTimeout(locationRequest.current); locationRequest.current = null; setLocating(false); onLocationState?.(e.code === 3 ? "timeout" : "denied"); }, { enableHighAccuracy: true, timeout: 8000, maximumAge: 60000 }); };
  const cancelLocation = () => { if (locationRequest.current !== null) { window.clearTimeout(locationRequest.current); locationRequest.current = null; setLocating(false); onLocationState?.("cancelled"); } };
  return <div className="omni-map-wrap"><div ref={container} className="omni-map" aria-label="Omni discovery globe" /><div className="omni-map-attribution">© OpenStreetMap contributors · © OpenMapTiles</div><div className="omni-map-controls" aria-label="Map controls"><button type="button" aria-label="Locate me" onClick={locate}>⌖</button><button type="button" aria-label="Zoom in" onClick={() => mapRef.current?.zoomIn()}>+</button><button type="button" aria-label="Zoom out" onClick={() => mapRef.current?.zoomOut()}>−</button>{locating && <button type="button" aria-label="Cancel location" onClick={cancelLocation}>×</button>}</div></div>;
}
