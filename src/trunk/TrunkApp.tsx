...
export function TrunkApp() {
  ...
  const loadSellerQueue = async () => {
    ...
    try {
      ...
      const [queueResult, catalogueResult] = await Promise.all([
...
function SellerCatalogueEditor(props: { products: SellerCatalogueProduct[]; state: 'idle' | 'loading' | 'error' | 'success'; error: string; onMutation: (action: 'create' | 'update', productId: string | undefined, fields: SellerOfferMutationFields) => void }) {
  const facilities = Array.from(new Map(props.products.map((product) => [product.facilityId, { id: product.facilityId, name: product.facilityName }])).values());
