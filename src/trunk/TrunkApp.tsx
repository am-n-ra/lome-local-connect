...
import { MenuIcon, RoleSwitch, SearchDock, StatusBadge, PriceBadge, FilterChip, FacilityCard, ContextPanel, facilityStatus, discountPercent } from './v3';

const emptySearchOptions: SearchOptions = { category: '' };

type Panel = 'none' | 'auth' | 'facility' | 'claim' | 'availability' | 'buyer-requests' | 'seller-entry' | 'field-pilot' | 'inbox' | 'reviewer' | 'admin-roles';
...
export function TrunkApp() {
  ...
  const submitAuth = async (event: FormEvent) => {
    ...
    try {
      ...
      setAppliedOptions(draftOptions);
      setAuthState('idle');
      const resumePanel = authReturn === 'availability' ? 'availability' : authReturn === 'buyer-requests' ? 'buyer-requests' : authReturn === 'seller-entry' ? 'seller-entry' : authReturn === 'field-pilot' ? 'field-pilot' : authReturn === 'claim' ? 'claim' : authReturn === 'inbox' ? 'inbox' : authReturn === 'reviewer' ? 'reviewer' : authReturn === 'admin-roles' ? 'admin-roles' : 'none';
      setAuthReturn('none');
  ...
  return (
    <main ref={appRef} className={`omni-stage-viewport ${mainClass}`} data-auth={authClient ? 'configured' : 'missing'}>
      <Suspense fallback={<div className="omni-map-loading" role="status" aria-live="polite"><span className="spinner" /> Chargement de la carte…</div>}><TrunkMap facilities={facilities} selectedId={selectedFacility?.id ?? null} onSelect={handleMapPinSelect} onBoundsChange={setBounds} onRevealStateChange={handleRevealStateChange} revealKey={searchRevealKey} contextSurfaceOpen={nearbyOpen || optionsOpen || menuOpen || panel !== 'none'} /></Suspense>
      ...
      {panel === 'none' && <>
        <div className="search-anchor omni-keyboard-aware">
          ...
          <SearchDock query={query} onQuery={setQuery} onSubmit={beginSearch} onFilters={() => { setOptionsOpen((open) => !open); setMenuOpen(false); }} filtersActive={optionsOpen} inputRef={searchInputRef} />
        </div>
