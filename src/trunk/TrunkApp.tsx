...
import { cancelFacilityClaim, confirmExternalPayment, createFacilityClaimDraft, createPurchaseIntent, declareExternalPayment, getAvailabilityResponses, getTransaction, getBuyerAvailabilityRequests, getClaimStorageStatus, getFacilityDetail, getNotificationInbox, getOperatorRuns, getReviewQueue, getSellerActivationQueue, getSellerAvailabilityQueue, importPublicFacility, issueQrToken, listPublicFacilities, markNotificationSeen, getWebPushStatus, rebindDemoSeller, requestAvailability, requestSellerAvailabilityResponse, reviewFacilityClaim, subscribeWebPush, activateSellerAccount, submitFacilityClaim, transitionTransaction, uploadFacilityEvidence, verifyQrToken } from './api';
...
export function TrunkApp() {
  ...
  const [sellerTab, setSellerTab] = useState<'requests' | 'catalogue' | 'transaction'>('requests');
  const [sellerTransactionId, setSellerTransactionId] = useState('');
  const [sellerQrState, setSellerQrState] = useState<'idle' | 'loading' | 'error' | 'success'>('idle');
  const [sellerQrError, setSellerQrError] = useState('');
  const [sellerQrResult, setSellerQrResult] = useState<{ transactionId: string; token: string; expiresAt: string } | null>(null);
  ...
  const [activationQueue, setActivationQueue] = useState<Array<{ accountId: string; authUserId: string; onboardingState: string; facilityCount: number; createdAt: string }>>([]);
  ...
  const issueSellerQr = async () => {
    if (!sellerTransactionId.trim()) {
      setSellerQrState('error');
      setSellerQrError('Indiquez le transaction ID remis par l’acheteur.');
      return;
    }
    setSellerQrState('loading');
    setSellerQrError('');
    try {
      const token = await getAuthToken();
      if (!token) {
        setSellerQrState('error');
        setSellerQrError('Votre session doit être réouverte avant d’émettre le QR.');
        return;
      }
      const result = await issueQrToken({ transactionId: sellerTransactionId.trim(), token });
      if (!result.ok || !result.data) {
        setSellerQrState('error');
        setSellerQrError(result.error?.message ?? 'Le QR ne peut pas être émis pour cette transaction.');
        return;
      }
      setSellerQrResult(result.data);
      setSellerTransactionState('qr_ready');
      setSellerQrPayload(JSON.stringify({ transactionId: result.data.transactionId, token: result.data.token }));
      setSellerQrState('success');
    } catch (caught) {
      setSellerQrState('error');
      setSellerQrError(caught instanceof Error ? caught.message : 'Le QR ne peut pas être émis pour le moment.');
    }
  };
...
function SellerWorkspaceSheet(props: { user: SessionUser | null; queue: SellerAvailabilityQueue | null; queueState: 'idle' | 'loading' | 'error'; queueError: string; request: SellerAvailabilityRequest | null; tab: 'requests' | 'catalogue' | 'transaction'; transactionId: string; setTransactionId: (value: string) => void; qrState: 'idle' | 'loading' | 'error' | 'success'; qrError: string; qrResult: { transactionId: string; token: string; expiresAt: string } | null; qrPayload: string; setQrPayload: (value: string) => void; verifyState: 'idle' | 'loading' | 'error' | 'success'; verifyError: string; onIssueQr: () => void; onVerifyQr: () => void; transactionState: TransactionState; paymentState: 'idle' | 'loading' | 'error' | 'success'; paymentError: string; transitionState: 'idle' | 'loading' | 'error' | 'success'; transitionError: string; onConfirmPayment: () => void; onAdvanceFulfilment: () => void; onAdvanceFulfilled: () => void; rebindState: 'idle' | 'loading' | 'error'; rebindError: string; onRebindDemo: () => void; setTab: (value: 'requests' | 'catalogue' | 'transaction') => void; responseStatus: SellerResponseStatus; setResponseStatus: (value: SellerResponseStatus) => void; quantity: number; setQuantity: (value: number) => void; price: string; setPrice: (value: string) => void; message: string; setMessage: (value: string) => void; responseState: 'idle' | 'loading' | 'error' | 'success'; responseError: string; responseResult: { status: AvailabilityResponseStatus; observedAt: string } | null; onLoadQueue: () => void; onSelectRequest: (request: SellerAvailabilityRequest) => void; onSubmitResponse: () => void; onBackToQueue: () => void; onClose: () => void; onSignOut: () => void }) {
