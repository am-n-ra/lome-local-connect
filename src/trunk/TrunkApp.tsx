...
function SearchOptionsPopover(props: { category: string; categoryOptions: string[]; setCategory: (value: string) => void; quantity: number; setQuantity: (value: number) => void; budgetMode: 'unlimited' | 'maximum'; setBudgetMode: (value: 'unlimited' | 'maximum') => void; budget: string; setBudget: (value: string) => void; onClear: () => void; onApply: () => void; onClose: () => void }) {
  return <section id="search-options" className="v3-filter-panel" role="region" aria-label="Filtres de recherche"><button className="v3-filter-close" type="button" aria-label="Fermer les filtres" onClick={props.onClose}><X size={16} /></button><h3>Filtrer</h3><div className="v3-filter-section"><span>Catégorie</span><div className="v3-chip-row"><FilterChip active={!props.category} onClick={() => props.setCategory('')}>Toutes</FilterChip>{props.categoryOptions.filter(Boolean).map((category) => <FilterChip key={category} active={props.category === category} onClick={() => props.setCategory(category)}>{category}</FilterChip>)}</div></div><div className="v3-filter-section"><span>Quantité minimale</span><div className="v3-chip-row">{[1, 2, 3, 5, 10].map((value) => <FilterChip key={value} active={props.quantity === value} onClick={() => props.setQuantity(value)}>{value} unité{value === 1 ? '' : 's'}</FilterChip>)}</div></div><div className="v3-filter-section"><span>Budget</span><div className="v3-chip-row"><FilterChip active={props.budgetMode === 'unlimited'} onClick={() => props.setBudgetMode('unlimited')}>Sans plafond</FilterChip><FilterChip active={props.budgetMode === 'maximum'} onClick={() => props.setBudgetMode('maximum')}>Budget maximum</FilterChip></div>{props.budgetMode === 'maximum' && <label className="money-input">Montant maximum<input type="number" min="0" step="0.01" value={props.budget} onChange={(event) => props.setBudget(event.target.value)} placeholder="0,00" /></label>}</div><div className="v3-filter-apply"><button className="primary-button omni-pressable" type="button" onClick={props.onApply}>Appliquer</button></div><button className="text-button filter-clear" type="button" onClick={props.onClear}>Effacer les filtres</button></section>;
}
