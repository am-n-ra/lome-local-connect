...
export async function getFacilityBonusStatus(input: { token: string; facilityId: string }): Promise<ApiResult<FacilityBonusStatus>> {
  const response = await fetchWithRecovery(`/api/v2/seller/facilities/${encodeURIComponent(input.facilityId)}/bonus`, { headers: { Accept: 'application/json', Authorization: `Bearer ${input.token}` } });
  return parse(response);
}

export async function getFacilityAnalytics(input: { token: string; facilityId: string }): Promise<ApiResult<SellerFacilityAnalytics>> {
  const response = await fetchWithRecovery(`/api/v2/seller/facilities/${encodeURIComponent(input.facilityId)}/analytics`, { headers: { Accept: 'application/json', Authorization: `Bearer ${input.token}` } });
  return parse(response);
}

export async function listFacilityAdCampaigns(input: { token: string; facilityId: string }): Promise<ApiResult<AdCampaignListResult>> {
  const response = await fetchWithRecovery(`/api/v2/seller/facilities/${encodeURIComponent(input.facilityId)}/campaigns`, { headers: { Accept: 'application/json', Authorization: `Bearer ${input.token}` } });
  return parse(response);
}

export async function createFacilityAdCampaign(input: { token: string; facilityId: string; name: string; budgetMinor: number; startsAt: string; endsAt: string; idempotencyKey: string }): Promise<ApiResult<AdCampaignCreateResult>> {
  const response = await fetchWithRecovery(`/api/v2/seller/facilities/${encodeURIComponent(input.facilityId)}/campaigns`, {
    method: 'POST',
    ...
    body: JSON.stringify({ name: input.name, budgetMinor: input.budgetMinor, startsAt: input.startsAt, endsAt: input.endsAt }),
  });
  return parse(response);
}
...
export async function getBulkPacks(input: { token: string }): Promise<ApiResult<BulkPack[]>> {
  const response = await fetchWithRecovery('/api/v2/buyer/credit-packs', {
    headers: { Accept: 'application/json', Authorization: `Bearer ${input.token}` },
  });
  return parse<BulkPack[]>(response);
}
export async function purchaseBulkPack(input: { token: string; packId: string; callbackUrl: string; customer?: { email?: string | null; firstName?: string | null; lastName?: string | null }; idempotencyKey: string }): Promise<ApiResult<WalletRechargeResult>> {
  const response = await fetchWithRecovery('/api/v2/buyer/credit-packs', {
    method: 'POST',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${input.token}`, 'Idempotency-Key': input.idempotencyKey },
    body: JSON.stringify({ packId: input.packId, callbackUrl: input.callbackUrl, customer: input.customer ?? {} }),
  });
  return parse<WalletRechargeResult>(response);
}
...
export async function createSellerFacility(input: { token: string; name: string; facilityType: FacilityType; ownerKind?: OfferOwnerKind; category?: string | null; description?: string | null; address?: string | null; latitude?: number | null; longitude?: number | null; rayonKm?: number | null; contactPhone?: string | null; contactWhatsapp?: string | null; idempotencyKey: string }): Promise<ApiResult<CreateSellerFacilityResult>> {
  const response = await fetchWithRecovery('/api/v2/seller/facilities', {
    method: 'POST',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${input.token}`, 'Idempotency-Key': input.idempotencyKey },
    body: JSON.stringify({ name: input.name, facilityType: input.facilityType, ownerKind: input.ownerKind ?? 'organisation', category: input.category ?? null, description: input.description ?? null, address: input.address ?? null, latitude: input.latitude ?? null, longitude: input.longitude ?? null, rayonKm: input.rayonKm ?? null, contactPhone: input.contactPhone ?? null, contactWhatsapp: input.contactWhatsapp ?? null }),
  });
  return parse(response);
}
...
export async function createSellerProductDraft(input: { token: string; facilityId: string; name: string; description?: string | null; unit?: string; prixOriginal: number; currency: string; pourcentageReduction: number; stockLoueOmni: number; idempotencyKey: string; positionKind?: OfferPositionKind | null; uniquenessKind?: OfferUniquenessKind | null; handoverKind?: OfferHandoverKind | null; priceKind?: OfferPriceKind | null; conditionKind?: OfferConditionKind | null }): Promise<ApiResult<{ productId: string; facilityId: string; publicationState: 'draft'; prixReduit: number }>> {
  const response = await fetchWithRecovery('/api/v2/seller/catalogue', {
    method: 'POST',
    headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${input.token}`, 'Idempotency-Key': input.idempotencyKey },
    body: JSON.stringify({ facilityId: input.facilityId, name: input.name, description: input.description ?? null, unit: input.unit ?? 'unit', prixOriginal: input.prixOriginal, currency: input.currency, pourcentageReduction: input.pourcentageReduction, stockLoueOmni: input.stockLoueOmni, positionKind: input.positionKind ?? null, uniquenessKind: input.uniquenessKind ?? null, handoverKind: input.handoverKind ?? null, priceKind: input.priceKind ?? null, conditionKind: input.conditionKind ?? null }),
  });
  return parse(response);
}
...
export async function getAdminSellerActivationQueue(input: { token: string }): Promise<ApiResult<{ candidates: Array<{ accountId: string; authUserId: string; onboardingState: string; facilityCount: number; createdAt: string; suspended: boolean }> }>> {
  const response = await fetchWithRecovery('/api/v2/admin/seller-activations', {
    headers: { Accept: 'application/json', Authorization: `Bearer ${input.token}` },
  });
  return parse(response);
}
export async function adminActivateSellerAccount(input: { accountId: string; token: string }): Promise<ApiResult<{ accountId: string; onboardingState: 'seller_ready'; activated: true }>> {
