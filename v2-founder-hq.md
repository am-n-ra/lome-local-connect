...
|---|---|---|---|---|---|---|
| Active milestone | The canonical Species remains in review. Canopy V4.2 is implemented and source-validated, with bounded local visual proof: the map now follows the Africa-globe white/black/gray reference, idle rotation is restricted to direct map interaction, primary globe drag follows a vertical-axis helper, minus/plus/recenter remain visible, the approximate-location band is removed, and passive arrival remains non-recentering. The provider remains the last known-good Liberty path with a calibrated globe silhouette; Positron was rejected locally because its direct-controller style never reached loaded vector tiles. | `v2-species.md`, `docs/maquette/omni-species-maquette.md`, `omni-v2-canopy-v4-contract-2026-08-24.md`, `v2-species-audit-2026-08-24.md`, `omni-v2-canopy-v4.1-local-visual-check.md` | `partial / V4.2 bounded validation; parent gate open` | Manus + owner review | Complete real-device/touch confirmation, native-pin result inspection and the remaining authenticated/accessibility/performance matrix. | Do not advance roles, OSM or pilot expansion while Species proof is open. |
...
| Buyer Species | The map-first shell, dock/sheet language and public-facility boundary remain intact. Bounded V4.2 local and prior canonical proofs confirm the reference white/black/gray globe, direct-map-only rotation ownership during search/Options/account focus, vertical-axis globe drag, permanent minus/plus/recenter controls, 16px mobile text controls, non-obstructive location status and reversible projection, plus the continent-to-local reveal zoom steps. Real-result native-pin movement and full authenticated compact/accessibility proof remain open. | `v2-species.md`, `docs/maquette/omni-species-maquette.md`, `v2-species-audit-2026-08-24.md`, `omni-v2-canopy-v4.1-local-visual-check.md`, Buyer proof record | `partial / V4.2 bounded validation; parent gate open` | Manus | Complete real-device/touch confirmation, native pin movement inspection and remaining compact/focus/recovery coverage; do not call multi-product availability complete. | Visual fixes must not break claim/availability ownership or public trust semantics. |
...
| Release / operations | V4.2 implementation commit `381756d4` and final evidence commit `1345c95` are on GitHub branch `omni-v2-rebuild`; the latest canonical READY deployment is `dpl_DXfz3X7ybMo8TaWnvUWBGrUVfHtf`, serving `omni.sparkafrika.online` with exactly 12 Node functions. | Git history, Vercel deployment metadata, `canopy-v4-2-canonical-smoke.md` | `controlled / READY; acceptance still open` | Manus | Resolve the blank final high-zoom reveal, then complete real-device, native-pin, authenticated accessibility and remote-tile checks before any Species gate decision. | Preserve exactly 12 functions and keep generated bundles out of commits. |
| Decisions | Stay in Species/Canopy rather than expanding Root/Trunk operations. The V4.2 correction is a client Canopy change: inherit the supplied Africa-globe reference, keep a white field with black ocean/light land, make rotation map-owned only, enforce the vertical-axis globe gesture, keep controls stable and remove the obstructive approximate-zone band. Multi-product availability remains a Root/API decision, not a cosmetic UI patch. | `v2-species.md` Species amendment; `docs/maquette/omni-species-maquette.md`; `omni-v2-canopy-v4-contract-2026-08-24.md`; `v2-species-audit-2026-08-24.md`; `v2-tasks.md` | `review now` | Owner + Manus | Complete V4.2 canonical deployment and the remaining real-device/native-pin/accessibility evidence; decide the multi-product request contract before implementing it. | Keep operational branches paused and do not represent synthetic geolocation or the planning note as marketplace capability. |

## Current gate

> **Species gate remains open, with Canopy V4.2 deployed and materially advanced but not accepted.** The source baseline passed with 122 tests across 18 files, the local 390×844 and 1024×880 read-only proofs passed, and deployment `dpl_8UondDSFQHjPKmdu8dY1GZajpV2a` confirmed the monochrome arrival/result shell and reversible controls. The canonical final reveal then reproduced a blank white map canvas at mercator zoom `12.80`, so vector street/boundary detail and native result-pin movement are not proven. The latest canonical READY smoke is recorded under deployment `dpl_DXfz3X7ybMo8TaWnvUWBGrUVfHtf`; the next smallest action is a properly verified MapLibre source-loading correction, followed by real-device/touch confirmation and authenticated compact/accessibility/performance proof. Multi-product availability is blocked behind a Root/API contract decision. Do not advance operational branches until these residual gates are recorded. No role assignment, OSM import, new evidence action, payment or transaction work is authorized by this board.
...
The read-only audit includes the Canopy V3/V4 evidence, the V4.1 implementation amendment, bounded local mobile/desktop proof and the canonical READY smoke check. Keep Global Root `review` and Species `partial` until real-device/touch, native-pin result, authenticated accessibility and remote-tile evidence support a gate decision.
...
## Auth session and bounded Buyer proof checkpoint — 2026-08-25

**Changed:** The client now accepts both supported response forms: the actual Better Auth vanilla client shape `data.user` and the Supabase-compatible adapter shape `data.session.user`. The previous mapper handled only the latter, so a successful vanilla sign-in was incorrectly surfaced as `Auth succeeded but no active session was returned.`

**Evidence:** Commit `00cf425` reached READY deployment `dpl_2fw9JW8D54o2PW1RbYwA9eMhionh`, with canonical alias `omni.sparkafrika.online` and exactly 12 Node functions. After the authorized login takeover and a clean reload, the canonical account menu changed to `Votre espace` and exposed the authenticated read-only entries `Mes demandes`, `Inbox Omni`, `Outils terrain Omni` and `Revue des claims`. Opening `Mes demandes` loaded the server-backed Buyer history surface with its safe-resumption notice. The source suite is now **127 tests in 19 files**, all passing.

**Boundary / next gate:** This proves session hydration and a bounded authenticated Buyer read surface; it does not prove any business write. No claim, availability request, Seller response, Reviewer decision, payment, QR, transaction or notification mutation was performed. Continue with compact focus/recovery and the remaining device/input/performance proofs. Keep OSM/Overpass enrichment, PWA/Web Push, Seller/Reviewer operations, payment, QR, transactions and multi-product behind their existing Root/API gates.

## Compact focus/back/recovery checkpoint — 2026-08-25

**Evidence:** From the authenticated canonical session, `Escape` closed `Mes demandes` and returned to the active map. Reopening the account menu kept `Votre espace` available. A non-pointer `Tab` moved focus from `Fermer le menu` to the first authenticated menu item `Mes demandes`; `Enter` opened the Buyer sheet. The server-backed history then settled to `Historique récent` with the safe `Reprise sûre` notice and `Retour à la carte`.

**Decision:** The compact authenticated Buyer focus/back/recovery slice is materially advanced and remains bounded. This is not full keyboard or screen-reader certification, and it does not authorize any business write. Real-device touch/permission, full accessibility traversal, native-pin density/movement and remote-tile/performance proofs remain the next Species/Canopy gate. OSM/Overpass enrichment, PWA/Web Push, Seller/Reviewer operations, payment, QR, transactions and multi-product remain paused behind their existing Root/API gates.

## Authenticated Buyer search/reveal checkpoint — 2026-08-25
...
## iPhone 142 follow-up and reveal synchronization — 2026-08-25

**Observed:** The post-0185e3d iPhone recording confirms that authentication now succeeds: the auth sheet closes after `Connexion...` and the previous active-session error is absent. The search dock remains the divergent surface: the user reports the whole Buyer shell lifting during search focus, unlike the auth sheet where only the active input surface responds. The reveal visibly reaches the region stage, then the results sheet appears before the final city/result framing is perceptible.
...
## Functional foundation re-route — 2026-08-25
**Decision:** Founder HQ bypasses the formal Species/Canopy closure sequence for delivery prioritization, not for security or production gates. The next workstream is the smallest usable end-to-end foundation: Buyer availability -> purchase intent -> shared transaction state -> Seller QR issuance/verification -> external payment declaration/confirmation -> fulfilment -> Buyer receipt confirmation.
**Implemented in this checkpoint:** authenticated transaction snapshot read for cross-session Buyer/Seller synchronization; Buyer payment declaration and receipt affordances; Seller payment confirmation and fulfilment transition affordances; PWA manifest and service worker shell for installability and future opt-in push delivery. Validation currently passes **130/130 tests**, TypeScript/server build, production build, client boundary clean and exactly 12 Vercel functions.
**Next functional rings:** Admin/reviewer claim review and account activation surfaces; Free/Pro entitlement display and enforcement; OSM/Overpass public-facility population for Lomé/Aflao with attribution and dedupe; PWA install prompt and a real push-subscription backend with VAPID/provider configuration.
**Notification boundary:** OSM is a map-data source, not an end-user notification channel. Omni inbox events remain the source of truth; Web Push can only be enabled after explicit device consent plus a configured push provider, subscription persistence and server-side delivery path. No fake notification success is permitted.
**Mobile decision:** The iPhone 143 recording passes dock-only keyboard lift and no-auto-zoom, but the final viewport containment correction remains device-validated only after the next fresh recording. Keep the physical iPhone gate open while functional work proceeds.
**Boundary:** Seller/Admin, OSM, plans/billing and push are now active foundation workstreams, but no payment, account-role, notification delivery or external API mutation is executed without authenticated contracts, idempotency, auditability and focused proof. 

## Functional foundation checkpoint — commit 8238ac4 — 2026-08-25
**Published:** Buyer purchase-intent creation now leads to a shared, authenticated transaction snapshot; Seller can issue/verify the QR, confirm external payment and advance fulfilment; Buyer polls the shared state and can declare payment or confirm receipt when server state permits. All mutations retain bearer authentication and existing idempotency/audit contracts.
**PWA:** Added installability baseline through manifest, Apple/mobile metadata and service worker registration. The worker is prepared to receive push payloads, but no server-side push subscription or VAPID delivery is claimed yet.
**Validation:** 130/130 tests, server TypeScript build, production Vite build, 12 generated Vercel functions, client boundary clean and `git diff --check` clean. Canonical cache-bust `foundation-8238ac4` recovered from its brief paint blank to READY with the Buyer globe and dock intact.
**Honest gaps:** Admin plan management/Free-Pro controls, automated OSM ingestion/refresh and real push delivery still require their own implementation and environment/provider gates. Existing OSM/Overpass discovery and inbox remain available; OSM is not a notification provider. Physical iPhone viewport certification remains open.

## PWA installability checkpoint — commit 3369381 — 2026-08-25
**Published:** The account menu now exposes `Installer Omni` when the browser emits the native install prompt, and reports installed state for standalone/iOS contexts. The manifest is served on the canonical with standalone display, mobile metadata and Omni icons; the service worker is registered after load and is prepared for cache fallback and push event rendering.
**Validation:** Canonical `pwa-3369381` recovered from the normal boot mask to the READY Buyer globe and stable dock. The manifest endpoint returned valid JSON with scope `/`, standalone display and both configured icon entries. Full validation remains 130/130 tests, server build, production build, clean client boundary and 12 Vercel functions.
**Remaining foundation gaps:** Actual iPhone install flow, persistent push subscription, VAPID/provider delivery, automated OSM ingestion/refresh and Free/Pro/Admin management still require their own proof and configuration. Existing Overpass discovery and Omni inbox are not equivalent to those missing delivery/management contracts.

## Free/Pro visibility checkpoint — commit 1ef11a6 — 2026-08-25
**Published:** Facility details now expose the authoritative server-backed plan state: `Plan Free · 5 offres maximum`, `Plan Pro · offres illimitées`, or `Pro expiré · publication limitée`. This is visibility and entitlement communication only; it does not fake checkout, billing activation or payment.
**Validation:** 130/130 tests, server build, production build with exactly 12 Vercel functions, client boundary clean and `git diff --check` clean. Canonical `plans-1ef11a6` recovered from the normal boot mask to the READY monochrome Buyer globe with dock and controls intact.
**Next smallest action:** Add a guarded Admin/Reviewer account and plan surface only if the server exposes the matching authenticated role/entitlement contract; otherwise keep the current domain invariants as the source of truth. OSM ingestion and real push delivery remain separate workstreams.

## OSM and PWA notification foundation checkpoint — commit be1f056 — 2026-08-25
**Published:** Nearby discovery now falls back to bounded Overpass data only when the authenticated/public Omni list returns no persisted facilities. OSM results carry an explicit external source marker, display attribution, and open as informational-only surfaces; they cannot invoke Omni detail, claim, availability or transaction routes until separately imported and verified.
...
## iPhone keyboard / launch readiness checkpoint — 2026-08-26

**Signal:** The supplied `Demoiphone144.mp4` shows the intended native interaction: when the keyboard opens, the globe/background, top role controls and zoom controls remain fixed; only the search dock moves above the keyboard, then returns to its bottom resting position after dismissal. The prior whole-application upward movement is therefore a real UX defect against the approved Species interaction contract, not an acceptable mobile variant.

**Changed:** The first UI pass had already introduced `useViewportInsets`, `--omni-vvh`, `--omni-keyboard`, `omni-keyboard-aware` surfaces and a fixed stage. Diagnosis found that the more-specific `.species-app[data-keyboard-open] .search-anchor` rule still consumed the stale zero-valued `--keyboard-inset`, overriding the new dock inset. Commit `42e2ac1` changes that rule to consume `--omni-keyboard`; the root stage remains `100svh`, `max-height: 100svh`, `overflow: hidden`, while the dock owns the keyboard offset. No business logic, database, Auth, payment, transaction or user data changed.

**Evidence:** `pnpm test` passes **136/136**; `pnpm build` succeeds and emits exactly 12 Vercel functions. Production deployment `dpl_5Bj6rtL9FhSTX8X2QMpm7i1dqjLm` for commit `42e2ac1` is `READY`, target `production`, with canonical alias `omni.sparkafrika.online`. On the canonical page at `1024×880`, the globe reached `mapStatus: ready`; a controlled 300px keyboard-inset probe measured root `top: 0`, `height: 880`, `overflow: hidden`, and dock `bottom: 314px`, confirming that the dock—not the application root—moves in the corrected contract. The probe was DOM-only and restored its temporary state; no business mutation occurred.
