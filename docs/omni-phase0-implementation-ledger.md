...
| User location | `MapCanvas.tsx` user marker lifecycle; `carte.tsx` location state and Permissions API | Verified/needs clean-permission acceptance | Browser permission is modeled separately from location result: `prompt` requests on arrival, `granted` refreshes coordinates, `denied` is explained honestly, and a high-contrast exact-position pin exists only after browser geolocation succeeds; unavailable/fallback states produce zero user markers. |
| Globe visual quietness | `MapCanvas.tsx` layers/labels, palette, and public discovery feed | Verified/needs acceptance regression | Resting discovery uses bounded source-backed facilities with native pin markers, suppressed dense labels, no visual cluster bubbles, and a preserved grey/black-water white-land globe against a light landing field; the unrelated flat raster fallback has been removed. |
...
| Persistent search dock | `SearchDock.tsx`, `carte.tsx` | Verified/needs first-page redesign acceptance | Dock now has named discovery, structured, location-context, and action/request rows with a measured CSS clearance contract; the no-results request surface replaces the action row rather than overlapping quantity/budget. Location copy now distinguishes browser `prompt`/`denied` from approximate market fallback. |
