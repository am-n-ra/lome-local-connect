...
| User location | `MapCanvas.tsx` user marker lifecycle; `carte.tsx` location state | Verified/needs clean-permission acceptance | A high-contrast exact-position pin exists only after browser geolocation succeeds and remains visible in resting and search framing; unavailable/fallback states produce zero user markers. |
...
| Persistent search dock | `SearchDock.tsx`, `carte.tsx` | Verified/needs first-page redesign acceptance | Dock now has named discovery, structured, location-context, and action/request rows with a measured CSS clearance contract; the no-results request surface replaces the action row rather than overlapping quantity/budget. |
