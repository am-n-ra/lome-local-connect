...
| Resting globe rotation | `MapCanvas.tsx` lines 432–453 | Verified/needs visual refinement | Horizontal bearing rotation exists; revised PRD requires a cleaner globe, visible user position, subtle current-continent emphasis, and no oversized geographic text. |
...
| Globe visual quietness | `MapCanvas.tsx` layers/labels | Partial | Facility labels are rendered as map symbol layers and native markers; current map palette needs a final visual pass for clean geography, continent labels/highlight, and gray/white boundary emphasis. |
