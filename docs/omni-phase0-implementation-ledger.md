...
| Globe visual quietness | `MapCanvas.tsx` layers/labels, palette, and public discovery feed | Verified/needs acceptance regression | Resting discovery uses bounded source-backed facilities with native pin markers, suppressed dense labels, no visual cluster bubbles, and a warm cream/soft-water landing palette; the unrelated flat raster fallback has been removed. |
