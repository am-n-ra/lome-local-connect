...
The buyer flow was reconciled against the master and implemented as a single explicit chain: result card/pin → facility detail → dedicated catalogue → product selection → availability setup → response comparison → purchase-intent handoff. The facility card now opens inspection only; `Voir les produits` opens the real facility catalogue; matched products are highlighted as `Correspond à votre recherche`; and `Vérifier la disponibilité` carries the selected catalogue product into the four-stage availability sheet without requiring name re-entry.

The buyer availability sheet now accepts a selected product, persists `productId` into the server request, displays `Produit`, `Portée`, `Contraintes` and `Réponses`, preserves product context across auth return, and keeps the existing manual/Pro scope rules. A dedicated `CleanBuyerCatalogSheet` reads the existing `getFacility` product payload and provides loading, empty, unavailable and retry-safe surfaces. The canonical master now includes the full screen sequence and state machines in the Cards/catalogue correction section.
