...
| 2026-09-02 | Founder said **"accept but…"**: **transaction chat not properly represented**; **"verify availability" button not properly designed/responsive**; **result-grid sheet rises too high** (sheet heights must be contextual); **universal back to previous screen/sheet from anywhere, unless specifically locked (transaction)**; **spaces must be role-contextual** (Buyer ≠ Seller ≠ Admin). | Maquette rebuilt: **CHAT sheet** (transaction-scoped thread: system events + buyer/seller bubbles + input + auto-reply, "pas un DM social"); **consistent responsive buttons** (full-width `.btn`, `.btn.sm`); **contextual sheet heights** (`h-low` search/pending/pay, `h-auto` results grid — no more over-rising, `h-mid` flows, `h-full` facility/chat/menu); **universal back** via `HISTORY` stack (‹ in dock, works everywhere; blocked only when `TXN_LOCKED` → dock shows ✕ Annuler); **role-contextual homes** (BUYERHOME "Vos demandes & transactions" vs SELLER "attention" vs ADMIN "Revue"; menu "Mon espace" routes per role). Verified in browser: back PENDING→AVAIL ✓, lock → no back ✓, chat send/reply ✓. | G-02b pending founder visual acceptance of refined maquette. |
