# Intra-Skill Plan — `/nature-way` — Omni Gate 1 (Seed reconciliation + SDM) → Gate 4 (Trunk, CLOSED 2026-09-03) → Gate 5 (Branches/UI, planned)
...
> **Gate owned:** Seed (closed) → Species (closed 2026-09-02) → Root (closed 2026-09-02) → Trunk (**closed 2026-09-03**, handoff `HO-OMNI-04`) → **Branches/UI (Gate 5, `planned` → `in_progress — desktop delta V1.3 implémenté branch-only (T-10w(, PUSH PROD = ordre fondateur explicite requis, spot-check humain reste `manual`)** → **re-ouvert `review_founder` 2026-09-05** (audit delta V1.3, fondateur: « on est assez loin maquette, même le motion de recherche n'est pas en place » — aucun code, décisions §4 de l'audit))**
...
| T-07 | Gate 4 | Trunk in order Admin → Seller → Buyer; each slice proven before the next | T-06 | `done` | T-07a/b/c prod-proven; see rows below |
...
| T-07b | T-07 | **Seller slice** — onboarding/claim (S1 ported), Offer editor (discount/allocation), availability setter + freshness expiry (proven @ `2d4b487`), availability inbox + auto-reply, StockEvent history (S5, proven @ `2d4b487`), wallet/Pro, QR verification, fulfilment side, chat | T-07a | `done` | **prod proof 8/8 PASS 2026-09-03** (correlation `6978e7ea-…`, identity `demo@seller.omni` → demo-rebind fixture) — `omni-trunk-seller-inventory-T07b-2026-09-03.md` |
| T-07c | T-07 | **Buyer slice** — map + constraint search (SearchDock rewritten @ `deb5072`), saved searches (RG-3), availability request w/ per-account credits, intent, QR issuance, delayed contact/itinerary, payment declaration, fulfilment, rating, chat | T-07b | `done` | **prod proof 8/8 PASS 2026-09-03** (correlation `9839cc09-…`, identity `demo@buyer.omni`); B19 saved-searches built + proven — `omni-trunk-buyer-inventory-T07c-2026-09-03.md` |
| T-07d | T-07 | **Deploy freshness guardrail** — prod bundle hash must match `omni-v2-rebuild` HEAD build before any slice proof is recorded as prod-verified | T-07 (parallel guardrail) | `verified at T-07a/T-07c production evidence (2026-09-03)` | Bundle prod `index-BCg5G0cM.js` === build local; T-07c routes deployed (saved-searches 401 anonymous → live), no UI change |
| T-08 | Gate 4 | Integrated proof: one real seller + one non-team buyer + one team operator complete `offer → availability → intent → QR → payment → fulfilment → StockEvent → qualifying_sales` flawlessly | T-07a–T-07c | `done` | **integrated proof 16/16 PASS 2026-09-03** (correlation `65488663-91f4-4347-ba46-4fd9d600991d`, commit `ebd9d80`) — `scripts/prove-v2-integrated.mjs` + `omni-trunk-integrated-proof-T08-2026-09-03.md`; boucle complète → état final `closed` (10 événements v2). Volet opérateur : console admin T-07a jugée suffisante par le fondateur (2026-09-03) |
| T-09 | Gate 4 | Clôture Gate 4 (Trunk) | T-08 | `done` | **Décision fondateur 2026-09-03 : la console admin (T-07a) suffit pour le volet opérateur → Gate 4 CLOSED** |
| T-10 | Gate 5 — Branches (UI) | Surfaces UI sur le tronc prouvé, selon la maquette unifiée acceptée : dock recherche contrainte, salle transaction (timeline + chat + QR), surface recherches sauvegardées (B19, backend déjà prouvé), fiches facilité/offre, sheets vendeur/admin — 4 largeurs, map toujours visible (R-03), vocabulaire `docs/design.md` | T-09 | `in_progress` (go fondateur 2026-09-05) | T-10…T-10r exécutés (G-05, commits `51c24a6`→`986fc8d`( ; T-10s conformité V1.3 + feedback fondateur (voir ligne suivante( |
| T-10s | T-10 | Conformité UI vs maquette V1.3 + feedback fondateur (HO-OMNI-05( : rôle switch Buyer+Seller par défaut, fiche Compte (profil(, sheets desktop centrées, core motions dock/fondu, vérification reveal gating | T-10r | `done` (2026-09-05( | `docs/nature-way/omni-ui-inventory-G05-T10s-2026-09-05.md` ; correctifs dans `sheet.tsx`, `styles.css`, `v3.css`, `TrunkApp.tsx`, `AccountProfileSheet.tsx` ; tsc ✅ 296/296 tests ✅ build ✅ |
| T-10w | T-10 | **Coquille desktop V1.3** (delta) — la maquette V1.3 fournie par le fondateur (`HO-OMNI-05`「la derniere version de la maquette est celle ci」) introduit une coquille bureau (`body.desktop`, ≥1040px( : recherche en **barre permanente haute**, résultats en **rail gauche** (346px(, autres sheets en **tiroirs dockés à droite**, dock en **rail latéral gauche** — absente du prod. Implémenté sur `omni-v2-rebuild` (option 1 alignement desktop, réversible,: `omni-sheet-search` + `omni-sheet-results` + `omni-results-surface` en JSX, prop `constraintsPersistent` dans `LiquidSearchDock` (chips visibles sans frappe sur la top bar,, bloc CSS `@media (min-width:1040px)` dans `v3.css`. Mobile/tablette inchangés. | T-10v | `done (branch-only`, **PUSH PROD = ordre fondateur explicite requis** (guardrail T-07d : bundle `index-BYPWhr5c.js` ≠ prod `index-CupUr-t1.js`( ; vérité test ✅ 296/296, `lint` ✅, `check:boundary` ✅, build ✅, preuve browser statique ✅ (chips contraintes visibles sans frappe sur top bar desktop( | **Register** `omni-maquette-v1.3-reference-register` mis à jour (commit `2eb02ab`); spec correspondante à étendre; **Gate 5 re-ouvert conditionnellement** pending poussée + spot-check humain 4 largeurs + review fondateur (`review_founder`). |
| T-11 | T-10 | **Récouverture Gate  ​5 — audit delta UI vs maquette V1.3** (déclencheur fondateur 2026-09-05( : « on est assez loin maquette et même le motion de recherche n'est pas en place »。) Verdict: la cinématique **existe en code** (`TrunkMap`, `map-reveal.ts`( et se déclenche sur prod (F2–F4(, mais ce sandbox la bloque (`mapStatus=error` → `searchRevealKey=null`( et la chorégraphie est **appauvrie** vs V1.3 (pas de pavé tilt/rotate, pins révélés d'un bloc, pas de stagger pins(‪. Gears: rolepill glissant, countmark « 206 », vignettes riches, rail pré-recherche élégant — absents/simplifiés‪. **Aucun code modifié.** Décisions fondateur §4 de l'audit (6 questions(. | T-10w | `review_founder` | `docs/nature-way/omni-species-reopen-audit-G5-2026-09-05.md`; desktop delta T-10w reste `done` (654ee22(. |
...
| 2026-09-05 | Founder fournit la maquette V1.3 avec coquille desktop (body.desktop, ≥1040px( — recherche barre haute, résultats rail gauche, sheets tiroirs droits, dock rail gauche — absente du prod. | Option 1 alignement desktop implémentée sur `omni-v2-rebuild` (T-10w( ; mobile/tablette inchangés. | Coquille desktop = NEW desktop pattern accepté sous option 1 (« continu »,; PUSH PROD = ordre fondateur explicite requis (guardrail T-07d (; register + spec mis à jour. |
...
| 2026-09-03 | **T-08 integrated proof PASS 16/16** (correlation `65488663-91f4-4347-ba46-4fd9d600991d`, commit `ebd9d80`) — boucle complète offre→dispo→réponse→intent→QR→paiement→fulfilment→received→rated→`closed` (10 événements v2), vrai vendeur `demo@seller.omni` + acheteur `demo@buyer.omni`. **Fondateur : la console admin (T-07a) suffit pour le volet opérateur → Gate 4 CLOSED ; « on peut continuer ».** | T-07, T-08, T-09 → `done`; Gate 4 CLOSED; nouveau T-10 `planned` = Gate 5 (Branches/UI) ; header + handoff mis à jour. | Gate 5 s'ouvre sur ordre explicite du fondateur (« go ») : surfaces UI sur le tronc prouvé selon la maquette unifiée, ordre Admin → Seller → Buyer conservé, 4 largeurs, map toujours visible (R-03). |
...
| HQ plan ID | `HQ-OMNI-2026-09-02` |
| Local plan ID | `NW-PROD-OMNI-01` |
| Active gate | Gate 4 — Trunk (**CLOSED 2026-09-03**) → Gate 5 — Branches/UI (`planned`, attend l'ordre du fondateur) |
| Closed/open/blocked/deferred | T-06 (Root) `done`; T-07a/b/c `done` (prod proofs 9/9, 8/8, 8/8); T-07d (deploy guardrail) `done`; T-08 (integrated proof) `done` (16/16, correlation `65488663-…`); T-09 (clôture Gate 4) `done` (décision fondateur 2026-09-03 : console admin suffit pour le volet opérateur); T-10 (Gate 5 UI) `planned`. Nothing `blocked`. |
...
| Evidence (as-of 2026-09-03) | `npm test` 44 files / **285 tests** @ `a642da1`; Species closed (unified maquette); Root 038 + 039 applied by founder; T-07a/b/c inventories + prod proofs (`omni-trunk-{admin,seller,buyer}-inventory-T07{a,b,c}-2026-09-03.md`); T-08 integrated proof (`omni-trunk-integrated-proof-T08-2026-09-03.md`, `scripts/prove-v2-integrated.mjs`, commit `ebd9d80`) — boucle complète offre→rated→`closed`, 10 événements v2. |
| Residual gap | Aucun sur le Trunk. Ouverts pour Gate 5 : surface UI recherches sauvegardées (B19, backend prouvé, pas d'UI), salle transaction UI (timeline + chat + QR), race de visibilité serverless rating-après-`received` (retry documenté, à traiter côté UI), `POST /api/v2/wallet` 404 en prod (catch-all à vérifier si la surface wallet est exposée). |
| Founder decision (2026-09-03) | (1) La maquette = unifiée acceptée (Species fermée). (2) **Console admin T-07a suffit pour le volet opérateur de T-08 → Gate 4 CLOSED.** |
| Owner / next smallest action | **Fondateur : dire « go » pour ouvrir Gate 5 (Branches/UI, T-10).** Nature Way : au « go », inventaire des surfaces UI existantes vs maquette unifiée (même méthode que les slices Trunk), ordre Admin → Seller → Buyer conservé. |
