...
| T-07a | T-07 | **Admin slice** — inventory current admin surfaces (role pill, sheets, `admin.tsx`) vs accepted A1–A8 set (console équipe, file de revue, confiance + état opérationnel, activation/suspension, rôles, correction compteur, sorties opérateur, audit); close gaps; prove slice (API + browser, 320/375/768/1280) | T-06 | `implemented` — **prod proof pending founder** | Gap table + close-out `docs/nature-way/omni-trunk-admin-inventory-T07a-2026-09-03.md`. 285/285 tests, build green (`index-BCg5G0cM.js`), boundary clean. Migration `039_v2_operational_state.sql` must be applied on Neon before push (deploy ordering constraint). Prod browser+API proof of admin chain = founder step after push |
...
| Owner / next smallest action | Founder: apply `039_v2_operational_state.sql` on Neon, then push (push = prod deploy), then prod proof of the admin chain closes T-07a. Nature Way: T-07b (Seller slice) starts after T-07a prod proof. |
