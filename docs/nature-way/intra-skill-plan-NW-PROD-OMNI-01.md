...
| T-08 | Gate 4 | Integrated proof: one real seller + one non-team buyer + one team operator complete `offer → availability → intent → QR → payment → fulfilment → StockEvent → qualifying_sales` flawlessly | T-07a–T-07c | `ready` | Proof record |
