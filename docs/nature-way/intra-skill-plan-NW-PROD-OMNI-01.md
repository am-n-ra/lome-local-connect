...
| T-07c | T-07 | **Buyer slice** — map + constraint search (SearchDock rewritten @ `deb5072`), saved searches (RG-3), availability request w/ per-account credits, intent, QR issuance, delayed contact/itinerary, payment declaration, fulfilment, rating, chat | T-07b | `ready` | Gap table + proof record à produire |
