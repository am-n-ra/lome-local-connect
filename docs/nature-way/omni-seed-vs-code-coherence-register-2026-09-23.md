...
| **C-3** | **Pro = par entité** (Seed §Modèle économique) | « Seller Pro par *facilité* » est une **contradiction à corriger** : Pro = par **entité** | `v2_entities.commercial_plan` **existe** (`058`) et `061` a ajouté `v2_facility_entitlements.entity_id` **+ backfill + FK** — **mais** la colonne `facility_id` reste et les écritures Pro écrivent encore dessus | `061` · `invariants.ts` · `trunk-repository.ts:3788` | **Métier** | ⚠️ **PARTIEL** — la **cible** existe et est backfillée ; le **chemin d'écriture** n'est pas encore basculé. **0 entité avec `commercial_plan <> 'free'`** → non prouvé en données |
...
| Verdict | **`D-C1` a été tranché (2026-09-23) ET exécuté (`058`→`061`), vérifié live.** `C-1`/`C-2`/`C-7` **clos** ; `C-4`/`C-5`/`C-6` **corrigés** (`R-4`) ; **`C-3` partiel**. Il ne reste **pas un choix** mais un **résidu d'exécution** (`R-A`…`R-D`). **La vraie cause du rond-point : les documents d'état n'ont pas suivi le code** — pas le produit. |
| Gap résiduel | **Le fond EST dans le code** (`v2_entities`, `entity_id`, `position_kind`). Le gap est **d'exécution partielle** (Pro non basculé, caractéristiques 0/16 remplies) **et de mémoire** (documents périmés). |
| Décision | ~~**D-C1 / D-C2 / D-C3**~~ — **déjà tranchée `D-C1`, déjà exécutée.** Redemander = rejouer le rond-point |
| Prochaine plus petite action | **`SP-VALIDATION`** (clôture Species, fondateur) → puis **Root : finir `R-A`…`R-D`** |
