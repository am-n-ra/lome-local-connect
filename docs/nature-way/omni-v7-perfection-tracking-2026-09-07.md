...
| T5 | **V-7a — seller map-first** | Découpage pur livré: `src/trunk/seller-workspace.ts` + tests (owned public pins depuis `ownedIds`, sélection facilité par pin/list véridique — jamais changement de route, labels d'action ordonnés état/demandes/catalogue/scanner/solde（ | `wip — découpage livré; wiring UI (hoist données dans TrunkAppV13 + strips map-first（ reste` | `ea71025`; tests 50 files/319; lint+boundary clean; build `index-BXtDUEoU.js`=== prod (aucun changement bundle — pas encore importé（ |
