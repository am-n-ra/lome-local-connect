...
| **Smallest critical journey** | Acheteur ouvre Omni → cherche selon ses contraintes (distance, heure, budget, quantité…) **ou** flâne sur la carte filtrée → voit l'offre (et les offres alentour, pas seulement celle qu'il connaît) → sait si elle est **disponible maintenant** (réponse vendeur, ou auto depuis le stock alloué) → décide (« je veux acheter ») → **QR émis dans le même geste** → vendeur verrouille (scan caméra **ou** validation dashboard) → paiement externe **enregistré** (moyen déclaré) → exécution confirmée des deux côtés → avis → **événement de stock** qui rend la disponibilité plus fiable. **Variante in-store (acquisition) :** un client déjà en boutique scanne le **QR public de l'entité** → la transaction passe par Omni → **remise appliquée**. |
...
| **S-19** | **Avantage promotionnel obligatoire pour publier sur Omni** (défaut : remise %). C'est l'**incitation centrale** : ce qui donne à l'acheteur — même physiquement devant le vendeur — une raison de passer par Omni, et ce qui finance la boucle traçable. Déjà en base (`discount_percent` + trigger `enforce_product_discount`). **Barème : (a) libre mais > 0** (le vendeur choisit son taux ; aucun minimum imposé). | confirmé (a) |
...
| **S-24** | **Le scan du code acheteur est strictement côté vendeur.** L'acheteur a une **icône scan** (dock) pour scanner le **QR public d'un lieu/offre en physique** et profiter de l'avantage. Le vendeur **peut** scanner le code de l'acheteur **ou** simplement valider la demande dans sa partie dédiée — **l'acheteur ne voit jamais** d'écran « scanner le code acheteur » ni le dashboard vendeur (voir S-27). | confirmé |
| **S-25** | **L'offre appartient à l'ENTITÉ** (organisation **ou** individu — même objet). On crée des **offres** ; elles appartiennent à l'entité qui les publie. Le **lieu (facility)** est *où* l'offre est disponible, pas *à qui* elle appartient. Il n'y a pas de « produit appartenant à une facilité » dans le modèle V2. | confirmé |
| **S-26** | **Chaque transaction génère une VERSION de l'offre** (prix + coupon gelés au moment T), portée par le snapshot transactionnel — donc le QR transactionnel permet de **retrouver qui a acheté quoi, à combien, quand, où**. Confirmé code : `v2_transaction_snapshots` porte `buyer_account_id`, `product_id`, `quantity`, `unit_price_minor`, `coupon_code`, `net_amount_minor`, `facility_id`, `created_at`. | confirmé (fait vérifié) |
| **S-27** | **Le dashboard vendeur et le scan du code acheteur sont STRICTEMENT côté vendeur.** L'acheteur ne voit **jamais** le dashboard vendeur ni l'écran « scanner le code ». **Côté vendeur :** dans sa partie dédiée, il voit que **la demande de dispo est maintenant une intention d'achat** ; **au clic, ça valide en auto et lui ouvre le chat de transaction de cette demande précise**. **Côté acheteur :** section dédiée où l'étape évolue → il voit **« Code validé »**, puis la transaction continue. | confirmé |
| **S-28** | **Toujours créer une entité, même pour un particulier à objet unique** (choix (a)). Le particulier **est** l'entité ; l'entité se crée en une étape quasi invisible — pas de second chemin. | confirmé (a) |
| **S-29** | **L'espace vendeur est progressif.** Sans entité : l'espace s'ouvre mais montre **l'état d'entrée uniquement** (Créer / Revendiquer). Les **outils** (catalogue, offres, transactions, dashboard, scanner) **se débloquent avec l'entité** — sans entité il n'y a rien à gérer. | confirmé |
| **S-30** | **La confiance porte sur l'ENTITÉ, jamais sur l'offre.** Confirmé code : `trust_state` existe **uniquement sur `v2_facilities`** ; il n'y a **aucun** `trust_state` sur les offres. Une offre **hérite** de la confiance de son entité. | confirmé (fait vérifié) |
| **S-31** | **Publier NE requiert PAS la vérification.** Créer (ou revendiquer) une entité débloque immédiatement la publication (badge « Non vérifié ») ; la vérification opérateur puis la confiance par l'usage sont des **couches gagnées ensuite**, jamais des préalables. Confirmé code : `createSellerFacility` écrit `trust_state='unconfirmed'`, et `offerLimitFor` autorise la publication pour `unconfirmed`/`confirmed`/`certified`. | confirmé (fait vérifié) |
