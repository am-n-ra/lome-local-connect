...
## 13. Canopy V4.1 amendment — monochrome reference and map-only motion, 2026-08-25

The map visual target is the established Omni **white/black/gray reference**, not the green-toned V4 treatment: white map field/background, near-black oceans, white/light continents and charcoal/gray country and continent contours. Roads and labels remain neutral where the vector style exposes them. Green/sepia tint, heavy gray wash and decorative colored selection glow are excluded from the Species map. Facility and cluster marks remain quiet and source-backed; any density rings inherit neutral grayscale semantics.
...
## 15. Canopy V4.3 visual/provider amendment — 2026-08-25

The owner’s new reference text confirms that the intended behavior is a MapLibre-native vector globe rather than a raster silhouette: OpenFreeMap Positron-style vector geography is the candidate provider, with a white outer field, dark ocean, light land, fine geographic contours and progressively denser labels/roads as the camera moves from world to local scale. Omni’s permanent exclusions still apply: no heavy selected-region highlight, no literal `Votre position` chip, no colored trust/stock implication in public pins, and no automatic mobile recenter.

The Trunk must explicitly resolve the MapLibre worker to a stable same-origin build asset before constructing the map. Provider readiness is not established by `style.load` alone: the visual `Carte active` state is allowed only after the map reaches the usable loaded/idle path with the vector source available. Provider failure displays an honest retry state; it must not silently replace the vector reference with a synthetic raster map. Facility markers remain native source/layers, and worldwide OSM/Overpass enrichment remains a separate Root/operations gate rather than an implied result of this visual amendment.

The V4.3 Species proof requires compact and desktop settled frames, a loaded vector source, visible globe and fine boundaries, reversible globe/mercator projection, progressive reveal labels, native pins/roads at local scale when real data is available, stable map-only motion ownership, permanent controls, and no overlap with the dock or contextual sheet. Until the real-result local detail and coverage boundaries are proven, the Species gate remains open.
