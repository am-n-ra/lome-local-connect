...
**Statut :** `implémenté_branch` — alignement desktop V1.3 implémenté sur `omni-v2-rebuild` (option 1 « continu » fondateur(, **PUSH PROD = ordre fondateur explicite requis** (guardrail T-07d : bundle `index-BYPWhr5c.js` ≠ prod `index-CupUr-t1.js`(. Tests ✅ 296/296, `lint` ✅, `check:boundary` ✅, build ✅, preuve browser statique ✅ (chips contraintes visibles sans frappe sur la top bar desktop(. **Restants :** (1( poussée prod après go fondateur, (2( spot-check humain 4 largeurs (mobile 390, tablette 768, laptop 1280, desktop ≥1440(, (3( rail vide pré-recherche = état vide élégant (polish(, (4( `aria-modal` des barres/rails desktop = dette sémantique (devrait être `region`/`complementary` sur desktop(.