...
Query the browser geolocation permission state on landing arrival. When it is `prompt`, invoke the native location permission through an idempotent non-blocking effect; when it is `granted`, refresh the real coordinate without expecting a prompt; when it is `denied`, explain that browser settings block the request and keep explicit retry/recovery available. Keep the globe visible while pending, show a clear exact-position pin only after browser success without moving the resting camera, and label denied/timeout/unsupported cases as approximate market fallback without misplacing or fabricating the user marker. A stale session flag must not suppress a still-`prompt` browser state. When exact location is absent, the map recenter control and final reveal stage must say approximate market rather than personal position. Use only compact dock retry/recovery treatment after denial; do not add a large arrival card. Implement the staged reveal only after a real or restored search, with black/near-black highlights, suppressed country/city labels, semantic pauses, reduced-motion behavior, camera framing, and result reveal callbacks. Verify that manual zoom and pan never start the reveal.
...
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Globe/location | Open `/carte` with a clean browser permission state; confirm the native prompt is requested on arrival without a blocking card, permission `granted` refreshes the real coordinate, `denied` is explained honestly, explicit retry remains available, exact marker only appears after success, approximate-market recenter/reveal labels are used without a real position, grey/black-ocean white-land globe projection, light surrounding field, horizontal center/longitude rotation, pause/resume, and no decorative substitute. |
