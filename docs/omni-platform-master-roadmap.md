...
**Immediate correction gate:** Permission-state-aware non-blocking location request on arrival (`prompt` requests, `granted` refreshes, `denied` explains browser blocking), truthful market fallback, an unmistakable exact user pin only after browser success, explicit retry independent of stale session flags, human-like horizontal center/longitude rotation, sparse real facility discovery at rest, no default country/city label wall, search-only staged reveal, black/near-black active boundaries, a light cream/white field surrounding the preserved grey/black-water and white-land globe, and a stable named-row dock layout with mutually exclusive request/result surfaces and measured responsive clearance.
