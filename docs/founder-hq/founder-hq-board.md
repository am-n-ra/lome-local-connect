...
> **Gate 5 — Branches/UI — CLOSED 2026-09-05 (conditionnel : spot-check humain 4 largeurs restant — item `manual`).** Surfaces UI conformes à la maquette unifiée V1.1 acceptée(travail T-10…T-10r déjà exécuté et intégrant les retours PC du fondateur(; reste à prouver la conformité finale(inventaire, 4 largeurs, `npm test`, build, prod(hash===build(, puis clore la gate. Nature Way en contrôle via la même route (`/nature-way`(. Branch rule locked( voir ci-dessus(. Canopy/Rings restent planned(Gate​​6 = Canopy/launch-readiness, en `watch`(.