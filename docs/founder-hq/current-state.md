...
| `SP-V2-01` | **Haute** | **Impasse de l'offre sans propriétaire** (`unclaimed` + offre publiée). Mesuré live : **203/206** lieux sans compte ; **2** sont **visibles à l'acheteur** (`trust_state` ∈ `certified`/`unconfirmed`) **et portent une offre publiée** (`Atelier Kegue` ×2, `Pharmacie du Port` ×1). Or **aucun vendeur ne peut répondre** : `getSellerAvailabilityQueue` joint `f.account_id = <sellerAccountId>` — un lieu sans compte n'apparaît dans **aucune** file vendeur. **Conséquence mesurée** : l'acheteur peut demander la disponibilité (le chemin d'écriture ne teste que `publication_state` + confiance, **pas** la propriété), la demande **expire en 15 min sans réponse possible**, et **l'acheteur paie 0 crédit pour un aller simple**. C'est une **impasse qui ment** sur le cœur du Seed (S-05 : les lieux de fond de carte sont `unclaimed`, « **sans promesse de stock** »). Les **200** lieux `unclaimed` restants sont **conformes** (0 offre publiée). **Cause racine : dette de données**, pas de code — les 3 offres sont des fixtures `public_import` du **2026-08-22**, antérieures à R-B (`bfc3b7c`), qui n'ont jamais eu de propriétaire. **Action : décision fondateur** (retirer les offres orphelines, ou les rattacher à une entité — inventer un propriétaire serait un mensonge). |
