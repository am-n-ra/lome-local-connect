...
-- Note: neon_auth.user.id is uuid; v2_accounts.auth_user_id is text — cast with ::text.
-- Status 2026-09-03: probe executed via serverless driver — kheirlissi@icloud.com
-- already holds roles admin+reviewer, both status='active'. No corrective INSERT needed.
...
from v2_accounts a
where a.auth_user_id = (select u.id::text from neon_auth.user u where u.email = 'kheirlissi@icloud.com' limit 1)
limit 1;
...
where ar.account_id = (select a.id from v2_accounts a where a.auth_user_id = (select u.id::text from neon_auth.user u where u.email = 'kheirlissi@icloud.com' limit 1) limit 1)
...
--    (Not needed as of 2026-09-03 — admin role already active.)
--
-- insert into v2_account_roles (account_id, role, status, granted_by_account_id, granted_at)
-- values (
--   (select a.id from v2_accounts a where a.auth_user_id = (select u.id::text from neon_auth.user u where u.email = 'kheirlissi@icloud.com' limit 1) limit 1),
--   'admin',
--   'active',
--   (select a.id from v2_accounts a where a.auth_user_id = (select u.id::text from neon_auth.user u where u.email = 'kheirlissi@icloud.com' limit 1) limit 1),
--   now()
