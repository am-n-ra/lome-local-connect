...
## 2026-08-25 — Canopy V4.2 Africa-globe reference checkpoint

The owner clarified that the intended map is the existing Omni globe visual: white surrounding field, near-black ocean silhouette, light/white continents and restrained charcoal/gray geographic contours. The previous green-toned treatment is rejected. The image’s very dark selected-region highlight and the literal `Votre position` chip are also rejected. This request remains within the existing Species/Canopy path and does not open a new Root or operations plan.

The implementation keeps Liberty as the last known-good remote path, preserves the Natural Earth low-zoom silhouette, and applies the reference treatment only to the globe projection through grayscale/inversion/brightness with a transparent MapLibre background. The normal mercator map remains non-inverted grayscale. The search reveal steps now use the origin/main precedent: world `1.05`, continent `2.15`, country `5.35`, region `8.25`, city/zone `11.25` and local result framing `14.2` or fit-bounds. The sequence crosses the `2.4` globe/mercator threshold deliberately.

The local settled proof now has actual visual frames at `390×844` and `1024×880`: the globe is visible, Africa/other land masses read light against the near-black ocean, the surrounding field is white, the neutral user marker has no permanent chip, and minus/plus/recenter remain visible together. The proof assertions remain green for 122 tests/18 files, direct map-only rotation, stable vertical-axis drag, idle resume outside the map, 16px text input, default visual viewport scale, reversible projection and zero visible HTML pin overlays. A prior Positron experiment using the origin/main glyph rewrite was not accepted because the direct controller never reached `isStyleLoaded` or requested PBF tiles; it was reverted to Liberty to preserve a visible map.

**Audit decision:** `partial / V4.2 materially advanced; Species gate remains open`. The visual frame is now reference-aligned at the bounded local proof level, but source-backed vector boundary/road detail after reveal, native facility pins on a real result while moving, device-native touch/input behavior, owner-browser permission UX, authenticated compact accessibility/focus recovery and remote-tile/performance evidence remain open. No facility CTA, claim, seller/reviewer decision, transaction, notification, OSM or Root/API mutation was performed.


## 2026-08-25 — Canopy V4.2 canonical release smoke

Commit `381756d4fe2909bf95c724c35aed8caea40cee61` was pushed on `omni-v2-rebuild` and reached READY as Vercel deployment `dpl_8UondDSFQHjPKmdu8dY1GZajpV2a`. The deployment metadata names project `omniview`, target `production`, source `git`, canonical alias `omni.sparkafrika.online`, and exactly 12 Node functions. The canonical read-only arrival frame shows `Carte active`, a white outer field, near-black globe/ocean, light Africa/land, permanent `Zoom arrière` / `Zoom avant` / `Utiliser ma localisation`, and no heavy region highlight or literal `Votre position` chip.

The reversible canonical plus/minus smoke passed. The approved query `Marche de Hanoukope` reached a real result sheet with `Le monde`, `Résultats pour « Marche de Hanoukope »`, `Nouvelle recherche`, `Affiner`, `Retour à la carte`, one public `Marche de Hanoukope` card and the non-invoked `Voir le lieu` action. The globe remained mounted during the result state and no facility, seller, claim, reviewer, transaction, payment, notification, OSM or Root/API mutation was performed.
...
## 2026-08-25 — Canopy V4.3 vector-globe reference checkpoint

The owner’s latest reference text formalizes the intended map as a MapLibre-native vector globe: white outer field, dark charcoal ocean, light continents, fine geographic contours, calm physical orbit and progressive world → continent → country → region/city → local zoom. Omni inherits that visual and behavioral direction while retaining the explicit exclusions of the heavy selected-region highlight and literal `Votre position` chip. The attached text’s worldwide OSM/Overpass enrichment and `4,067+` coverage statement are not treated as proven or opened as an operations scope.

The provider diagnosis found that the browser and OpenFreeMap Positron are viable, but the bundled MapLibre worker was not resolved in the Omni Vite path. A Vite-managed same-origin worker asset was added and set before map construction. The live Omni preview then reached `load/idle`, loaded Positron glyph/PBF resources, rendered the Africa-facing globe and reported `Carte active` without requesting the synthetic raster fallback. The V4.3 palette removes the globe inversion and Liberty Natural Earth treatment and uses Positron vector layers directly.
...
**Audit decision:** `partial / V4.3 materially advanced; Species, Canopy and Ring gates remain open`. Canonical deployment of the worker/Positron source change, real-result local streets/neighborhoods and native facility-pin movement during the final reveal, production provider monitoring, authenticated accessibility/focus recovery, real-device touch and broader coverage remain open. The mono-product availability contract and all Root/business/privacy boundaries are unchanged.
