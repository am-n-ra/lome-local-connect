...
- G-02b (Admin/operator maquette set): **ready, pending founder visual acceptance** → `docs/maquette/omni-admin-operator-maquette.html` (served live at `https://work-1-ydrtpryguzosdbth.prod-runtime.all-hands.dev/omni-admin-operator-maquette.html` via `python3 -m http.server 12000` in docs/maquette) + spec `docs/nature-way/omni-admin-operator-maquette-set-G02b-2026-09-02.md`.
...
- **REFINEMENTS (accept-but, 2026-09-02):** **transaction chat** = dedicated CHAT sheet (system events + bubbles + input, scoped to transaction, "pas un DM social"); **consistent responsive buttons**; **contextual sheet heights** (`h-low`/`h-auto`/`h-mid`/`h-full` — grids/search never over-rise); **universal back** via history stack from anywhere, **blocked only when transaction locked** (dock then shows ✕ Annuler); **role-contextual spaces** (Buyer home "demandes & transactions" ≠ Seller "attention" ≠ Admin "Revue").
