...
try {
  for (const width of widths) {
    ...
    results.push({ width, canvas, attribution, resultsCount, overflow, errors });
    await page.screenshot({ path: `/tmp/omni-v1-${width}.png`, fullPage: true });
    await page.close();
  }
} finally { await browser.close(); }
console.log(JSON.stringify(results, null, 2));
if (results.some((result) => !result.canvas || !result.attribution || result.resultsCount === 0 || result.overflow || result.errors.length)) process.exit(1);
