...
| Responsive layout | Latest non-mutating canonical run at 320×760, 375×812, 768×900 and 1280×900 found a real MapLibre canvas, three public cards, no horizontal overflow, no topbar/control collision, no dock/sheet overlap and no dock/result-rail overlap; observed dock-to-sheet gaps were 8px on mobile and 10px at the broad desktop viewport. | `partial` | Buyer structural spacing is evidenced; Seller four-width states and interactive reachability remain open. |
| Keyboard / focus | The pin/cluster overlay exposes native accessible buttons; canonical Seller request detail Escape now returns to the Seller queue, and the corrected path is covered by `resolveEscape` tests. A full Buyer Tab/Shift+Tab/focus-trap pass remains open. | `partial` | Complete keyboard/focus and reduced-motion review across Buyer and Seller surfaces. |
