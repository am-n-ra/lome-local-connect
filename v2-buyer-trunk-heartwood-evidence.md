...
**Buyer Heartwood status:** `partial / V4.3 vector-globe path locally proven, canonical deployment pending`. Real-result streets/neighborhoods and native facility pin movement at final local zoom remain the next proof. Authenticated compact focus/recovery, device-native touch/input zoom, owner permission UX and remote performance remain open. No availability, claim, Seller, Reviewer, transaction, payment, QR, OSM or multi-product mutation occurred; the mono-product contract is unchanged.
