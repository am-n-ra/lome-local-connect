...
|---|---|---|---|---|
| Browser/server boundary | Defined in Root System | TypeScript/Vite build, Vercel bundling, `check:boundary`, canonical public/protected API smoke and branch JWKS reachability pass; connected-browser inspection returned HTTP 504 | `review` | Re-run browser bundle/interaction inspection when bridge responds |
| Neon Auth identity linking | Defined as idempotent and identity-preserving | Read-only check reports 35 `neon_auth.user` rows and 0 V2 accounts; branch Auth config includes the canonical trusted origin and reachable EdDSA JWKS; canonical unauthenticated POST returns HTTP 401; no bearer/provisioning session was proven | `review` | Live sign-in, bearer verification and duplicate-provisioning proof |
...
| QR and external payment | Hashed expiring server token, replay-safe verification and declaration-only external payment defined | Pure QR policy proves seller membership, match, expiry and replay denials; no atomic Neon mutation, live QR or payment proof | `review` | Conditional transaction/row-lock replay proof, authenticated seller session and recovery |
