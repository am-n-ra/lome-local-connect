...
| Persistence and migration | Additive, namespaced and preservation-first | Read-only branch catalog reports 26 public `v2_` tables, 125 constraints and legacy public tables coexisting; migration 003 executed on expiring disposable branch and verified with 2 constraints, 7 triggers and QR function presence; production/default branch was untouched | `review` | Preserved-row comparison, representative behavior tests and rollback/recovery record |
...
| Wallet and entitlements | One rechargeable Omni Wallet, append-only ledger, facility-scoped Pro and locked bonus defined | Disposable migration verifies append-only trigger presence; no wallet ledger mutation executed | `review` | Representative update/delete denial, ledger invariant, replay, failed recharge and non-withdrawal proof |
| Intent and transaction | Immutable snapshot, server transition, protected room and resumability defined | Pure transaction state graph and actor-membership policy pass; live persistence, event idempotency and recovery remain open | `review` | Persisted state transition, duplicate/concurrency and recovery proof |
| QR and external payment | Hashed expiring server token, replay-safe verification and declaration-only external payment defined | Pure QR policy plus disposable migration function/constraint presence verified; no representative atomic QR mutation, live QR or payment proof | `review` | Conditional transaction/row-lock behavior, concurrent replay proof, authenticated seller session and recovery |
