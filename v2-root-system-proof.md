...
| Intent and transaction | Immutable snapshot, server transition, protected room and resumability defined | Pure transaction state graph and actor-membership policy pass; disposable branch denies forged buyer intent and duplicate transaction state event; live persistence, idempotency response replay and recovery remain open | `review` | Persisted state transition, duplicate/concurrency and recovery proof |
