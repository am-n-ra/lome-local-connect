...
| Seller | One free Facility Slot; one owned facility; up to 5 published catalogue offers after the applicable trust/publication gates; response to demand; transaction handoff and QR scan; basic public offer | **10 USD / facility / 30 days** or **100 USD / facility / 12 months** from Omni Wallet; unlimited products for that facility; catalogue bulk edit; promotion/coupon tools; availability/transaction activity summary; named advanced tools | No certification shortcut, no `confirmed` badge, no stock guarantee, no Seller payout or withdrawal |
| Account capacity | One free Facility Slot per account; a single account can manage multiple companies and multiple facilities | Additional Facility Slots are account-scoped capacity entitlements purchased from the account Wallet. Each facility has its own independent Pro entitlement and billing lifecycle. | A slot does not grant Pro, trust, publication or payout; Pro on one facility never covers another facility |

The pricing is intentionally simple: **Buyer Pro is account-scoped; Seller Pro is facility-scoped; capacity is account-scoped; trust remains evidence/sales-scoped.** A Seller can be Free and still operate the basic demand/hand-off journey. Pro expands capacity and tools; it must never become a pay-to-trust mechanism. A single account may manage several companies and facilities, while each facility carries its own independent Pro entitlement and renewal state.

### Wallet recommendation

Use one account Wallet with a location-aware display currency. At launch, show XOF for Togo/Benin, GHS for Ghana and EUR for France, with an explicit fallback and no silent conversion. Wallet funds are confirmed only after a verified provider event. The ledger must distinguish `recharge`, `facility_pro_spend`, `buyer_pro_spend`, `slot_spend`, `promotion_spend`, `bonus_grant`, `reversal` and `refund_credit`. No balance can be withdrawn, transferred to another user or interpreted as Seller earnings. Recommended payment boundary: hosted FedaPay checkout for XOF recharge in the pilot, followed by server-side provider lookup and verified webhook reconciliation; do not auto-charge cards or process Buyer-to-Seller settlement in V1. Pro renewal should be explicit opt-in and consume the exact facility subscription amount from the Wallet only when sufficient funds exist.
...
A discount is a **mandatory authoritative offer value**, not a client-entered promise added later. Every Seller product draft must include either a percentage reduction or a fixed-amount reduction funded and honoured by the Seller. The server validates that the reduction is positive, does not reduce the net price below zero, and is compatible with the product currency; it then creates an immutable snapshot at intent creation. The recommended default for onboarding is a percentage offer because it adapts naturally to XOF, GHS and EUR display, while fixed-amount offers remain supported for sellers who price with round local-currency amounts.
