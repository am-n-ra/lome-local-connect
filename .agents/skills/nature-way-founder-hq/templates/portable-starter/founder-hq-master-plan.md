...
| Order | Gate | Decision condition | Primary specialist | Required resources | Expected return | Status | Re-plan trigger |
|---|---|---|---|---|---|---|---|
| 1 | `<Seed / Species / lifecycle / capital / opportunity / learning>` | `<what must be true>` | `/<skill>` | `<exact references/templates>` | `<gate, evidence, gap, owner, next action>` | `ready` | `<event>` |

## Activation ledger

| Handoff ID | Specialist | Activation status | Gate owned | Resource Receipt | Returned evidence | Next action |
|---|---|---|---|---|---|---|
| `<ID>` | `/<skill>` | `activated / user invocation required / unavailable` | `<gate>` | `<loaded / instantiated / not loaded>` | `<proof or gap>` | `<action>` |
...
Only the current gate and its ready work may be active. Downstream specialists remain `planned` until the current specialist returns the required handoff and Founder HQ reconciles the plan. `activated` means real control was transferred; a printed route is not activation. `done` requires evidence, not effort or an attractive output.

## Reconciliation log

| Date | Return or new fact | Plan changes | Decision | Owner | Next review |
|---|---|---|---|---|---|
